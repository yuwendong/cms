...
class Amazonus {
		...
		if (!defined('HTTPS_CATALOG')) {
			...
			if ($this->openbay->addonLoad('openstock') == true) {
				...
				foreach ($osCases as $osCase) {
					$amazonusSkuRows = $this->getLinkedSkus($osCase['pid'], $osCase['var']);
					foreach($amazonusSkuRows as $amazonusSkuRow) {
						$quantityData[$amazonusSkuRow['amazonus_sku']] = $osCase['qty_left'];
					}
				}
	...
	public function caseUpdateListen($caseId, $data) {
		...
		if ($this->openbay->addonLoad('openstock') && (isset($data['has_option']) && $data['has_option'] == 1)) {
			...
			foreach($data['case_option_stock'] as $optStock) {
				$amazonusSkuRows = $this->getLinkedSkus($caseId, $optStock['var']);
				foreach($amazonusSkuRows as $amazonusSkuRow) {
	...
	public function updateOrder($orderId, $orderStatusString, $courier_id = '', $courierFromList = true, $tracking_no = '') {
		...
		$amazonusOrderCases = $this->model_openbay_amazonus->getAmazonusOrderedCases($orderId);
		...
		foreach ($amazonusOrderCases as $case) {
			$newOrderItem = $orderItemsNode->addChild('OrderItem');
			$newOrderItem->addChild('ItemId', htmlspecialchars($case['amazonus_order_item_id']));
	...
	public function putStockUpdateBulk($caseIdArray, $endInactive = false){
		...
		foreach($caseIdArray as $caseId) {
			$amazonusRows = $this->getLinkedSkus($caseId);
			foreach($amazonusRows as $amazonusRow) {
				...
				if(!empty($caseRow)) {
					if($endInactive && $caseRow['status'] == '0') {
						...
						$quantityData[$amazonusRow['amazonus_sku']] = $caseRow['quantity'];
					}
				}
			}
		}
	...
	public function deleteCase($case_id){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "amazonus_case_link` WHERE `case_id` = '" . $this->db->escape($case_id) . "'");
	}
