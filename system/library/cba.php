...
class CBA {
			...
			foreach ($list as $list_item) {
				...
				foreach ($report_xml->Message as $message) {
					...
					foreach ($message->OrderReport->Item as $item) {
						...
						$order_case_id = (string)$item->SKU;

						$db->query("REPLACE INTO `" . DB_PREFIX . "order_amazon_case` (`order_case_id`, `amazon_order_item_code`) SELECT `op`.`order_case_id`, '" . $db->escape($amazon_order_item_code) . "' FROM `" . DB_PREFIX . "order_case` `op` WHERE `op`.`order_case_id` = '" . $db->escape($order_case_id) . "'");
					}
				}
	...
	public function orderCanceled($order) {
		...
		foreach ($order['cases'] as $case) {
			$flat .= $order['amazon_order_id'] . "\t\tGeneralAdjustment\t" . $case['amazon_order_item_code'] . "\n";
		}
...
';
		...
		foreach ($order['cases'] as $case) {
			$xml .= '
	  <Item>
		<AmazonOrderItemCode>' . $case['amazon_order_item_code'] . '</AmazonOrderItemCode>
		<Quantity>' . $case['quantity'] . '</Quantity>
	  </Item>
';
		}
	...
	public function setPurchaseItems($parameters) {
		...
		$i = 1;
		foreach ($parameters['cases'] as $case) {
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.MerchantId'] = $this->getMerchantId();
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.MerchantItemId'] = $case['model'];
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.SKU'] = $case['model'];
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.Quantity'] = $case['quantity'];
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.Title'] = $case['title'];
			$url_params['PurchaseItems.PurchaseItem.' . $i . '.UnitPrice.Amount'] = $case['price'];
