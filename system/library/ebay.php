...
final class Ebay {
	...
	public function getEbayItemId($case_id) {
		$this->log('getEbayItemId() - Case ID: '.$case_id);

		$qry = $this->db->query("SELECT `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `case_id` = '".(int)$case_id."' AND `status` = '1' LIMIT 1");
	...
	public function getEndedEbayItemId($case_id) {
		$this->log('getEndedEbayItemId() - ID: '.$case_id);

		$qry = $this->db->query("SELECT `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `case_id` = '".(int)$case_id."' AND `status` = '0' ORDER BY `ebay_listing_id` DESC LIMIT 1");
	...
	public function removeItemByCaseId($case_id) {
		$this->log('removeItemByCaseId() - ID: '.$case_id.'');

		$this->db->query("UPDATE `" . DB_PREFIX . "ebay_listing` SET `status` = '0' WHERE `case_id` = '".(int)$case_id."'");

		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `case_id` = '".(int)$case_id."'");
	}

	public function deleteCase($case_id) {
		$this->log('deleteCase() - ID: '.$case_id);

		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_listing` WHERE `case_id` = '".(int)$case_id."'");
	 ...
	 * @return array ([case id] = ebay item id)
	 */
		$this->log('getLiveListingArray()');

		$qry = $this->db->query("SELECT `case_id`, `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = '1'");

		$data = array();
		if($qry->num_rows > 0) {
			foreach($qry->rows as $row) {
				$data[$row['case_id']] = $row['ebay_item_id'];
			}
		}
	...
	public function getEndedListingArray() {
		...
		$qry = $this->db->query("SELECT e.* FROM (SELECT `case_id`, MAX(`ebay_listing_id`) as `ebay_listing_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = 0 GROUP BY `case_id`) `a` INNER JOIN `" . DB_PREFIX . "ebay_listing` `e` ON (`e`.`ebay_listing_id` = `a`.`ebay_listing_id`)");
	...
	public function getLiveCaseArray() {
		/**
		* Returns the list of linked items with eBay from the database
		* @return array ([ebay item id] = case id)
		*/
		$qry = $this->db->query("SELECT `case_id`, `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = '1'");
		...
		if($qry->num_rows) {
			foreach($qry->rows as $row) {
				$data[$row['ebay_item_id']] = $row['case_id'];
			}
		}
	...
	public function ebaySaleStockReduce($case_id, $sku = null) {
		/**
		* Gets the case info from an ID and sends to ebay update method.
		*/
		$this->log('ebaySaleStockReduce() - Is stock update needed (Item ID: '.$case_id.',SKU: '.$sku.')');

		if(!empty($case_id)) {
			if($sku == null) {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case` WHERE `case_id` = '".(int)$case_id."' LIMIT 1");
				$this->log('ebaySaleStockReduce() - Send item ID: "'.$case_id.'", Stock: "'.$query->row['quantity'].'" to decideEbayStockAction()');
				$this->decideEbayStockAction($case_id, $query->row['quantity'], $query->row['subtract']);
			}else{
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case_option_relation` WHERE `case_id` = '".(int)$case_id."' AND `var` = '".$sku."' LIMIT 1");
				$this->log('ebaySaleStockReduce() - Send item ID: '.$case_id.', VAR: '.$sku.', passing '.$query->row['stock'].' to decideEbayStockAction()');
				$this->decideEbayStockAction($case_id, $query->row['stock'], $query->row['subtract'], $sku);
			}
		}
	}
	...
	public function orderNew($order_id) {
		...
		if(!$this->isEbayOrder($order_id)) {
			if ($this->openbay->addonLoad('openstock') == true) {
				$this->log('orderNew() - Loop over cases (with OpenStock)');

				$os_array = $this->osCases($order_id);
				...
				$this->log('orderNew() - Loop over cases (no OpenStock)');
				foreach ($order_case_query->rows as $case) {
					$this->ebaySaleStockReduce((int)$case['case_id']);
				}
			}
		}
	}

	private function osCases($order_id) {
		$this->log('osCases() - Getting cases from');
		$order_case_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_case WHERE order_id = '".(int)$order_id."'");
		...
		foreach ($order_case_query->rows as $order_case) {
			...
			if (isset($case_query->row['has_option']) && ($case_query->row['has_option'] == 1)) {
				$case_option_query = $this->db->query("
				...
				if ($case_option_query->num_rows != 0) {
					$case_options = array();
					foreach ($case_option_query->rows as $case_option_row) {
						$case_options[] = $case_option_row['case_option_value_id'];
					}

					$var = implode(':', $case_options);

					$response[] = array('pid' => $order_case['case_id'], 'qty' => $order_case['quantity'], 'var' => $var);
				}
			} else {
				$response[] = array('pid' => $order_case['case_id'], 'qty' => $order_case['quantity'], 'var' => null);
			}
		}
	...
	public function disableCase($case_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "case` SET `status` = 0 WHERE `case_id` = '".(int)$case_id."' LIMIT 1");
	}

	public function disableVariant($case_id, $sku) {
		$this->db->query("UPDATE `" . DB_PREFIX . "case_option_relation` SET `active` = 0 WHERE `case_id` = '".(int)$case_id."' AND `var` = '".$this->db->escape($sku)."' LIMIT 1");
	}
	...
	public function putStockUpdate($item_id, $stock, $sku = null) {
		...
		$case_id = $this->getCaseId($item_id);
		$reserve    = $this->getReserve($case_id, $item_id, ($sku != null ? $sku : ''));
		...
		if($listing['status'] == 1 ) {
			...
			if($sku == null) {
				...
				if ($stock <= 0) {
					if ($this->config->get('ebay_disable_nostock') == 1) {
						$this->disableVariant($case_id, $sku);
					}
				}
	...
	public function putStockUpdateBulk($case_id_array, $endInactive = false) {
		// We know is that these case ID's have been modified. They should only be passed if the stock has changed so we can assume this.
		...
		foreach($case_id_array as $case_id) {
			if(array_key_exists((int)$case_id, $live_data)) {
				//case has been passed and is linked to active item
				$linkedItems[] = array('caseId' => (int)$case_id, 'itemId' => $live_data[$case_id]);
			}elseif(array_key_exists((int)$case_id, $endedData)) {
				//case has been passed and is not currently active
				$linkedEndedItems[] = array('caseId' => (int)$case_id, 'itemId' => $endedData[$case_id]);
			}else{
				//case does not exist in live or ended links so has never been linked.
			}
		}
		...
		foreach($linkedEndedItems as $item) {
			if($openstock == true) {
				$options = $this->model_openstock_openstock->getCaseOptionStocks($item['caseId']);
			} else {
				$options = array();
			}
			...
			if(empty($options)) {
				...
				$local_stock = $this->getCaseStockLevel($item['caseId']);

				if((int)$local_stock['quantity'] > 0 && $local_stock['status'] == 1) {
					//case has stock and is enabled, so re list it.
					$reserve = $this->getReserve($item['caseId'], $item['itemId']);
					...
					if($this->config->get('openbaypro_relistitems') == 1) {
						...
						$this->relistItem($item['itemId'], $item['caseId'],(int)$local_stock['quantity']);
					}
				}
			}else{
		...
		foreach($linkedItems as $item) {
			...
			if(!isset($ebay_listings[$item['itemId']])) {
				...
				if($endInactive == true && $local_stock['status'] == 0) {
					...
					//get any options that are set for this case
					if($openstock == true) {
						$options = $this->model_openstock_openstock->getCaseOptionStocks($item['caseId']);
					} else {
						$options = array();
	...
	public function getCaseStockLevel($caseId, $sku = '') {
		$this->log('getCaseStockLevel() - ID: '.$caseId.', SKU: '.$sku);

		if($sku == '' || $sku == null){
			$qry = $this->db->query("SELECT `quantity`, `status` FROM `" . DB_PREFIX . "case` WHERE `case_id` = '".(int)$caseId."' LIMIT 1");

			return array('quantity' => (int)$qry->row['quantity'], 'status' => ($qry->row['status']));
		}else{
			$qry = $this->db->query("SELECT `stock`, `active` FROM `" . DB_PREFIX . "case_option_relation` WHERE `case_id` = '".(int)$caseId."' AND `var` = '".$this->db->escape($sku)."' LIMIT 1");

			return array('quantity' => (int)$qry->row['stock'], 'status' => ($qry->row['active']));
		}
	}

	public function caseUpdateListen($case_id, $data) {
		$this->log('caseUpdateListen()');
		//check if there is an active item link
		$item_id = $this->getEbayItemId($case_id);
		if($item_id != false) {
			...
			if ($this->openbay->addonLoad('openstock') && (isset($data['has_option']) && $data['has_option'] == 1)) {
				...
				$variants           = $this->model_openstock_openstock->getCaseOptionStocks($case_id);
				$groups             = $this->openbay->getCaseOptions($case_id);
				$varData['groups']  = array();
				$varData['related'] = array();

				foreach($groups as $grp) {
					$t_tmp = array();
					foreach($grp['case_option_value'] as $grp_node) {
						$t_tmp[$grp_node['option_value_id']] = $grp_node['name'];

						$varData['related'][$grp_node['case_option_value_id']] = $grp['name'];
					}
				...
				foreach($variants as $option) {
					...
					$reserve = $this->getReserve($case_id, $item_id, $option['var']);
					if($reserve != false) {
						$this->log('caseUpdateListen() / Variant ('.$option['var'].') - Reserve stock: '.$reserve);
				...
				if($stock == true) {
					$this->log('caseUpdateListen() - Sending to API');
					$response = $this->call('item/reviseStockVariants', $varData);
					return $response;
				} else {
					$this->log('caseUpdateListen() - Ending item');

					$this->endItem($item_id);
				}
			}else{
				$this->decideEbayStockAction($case_id, $data['quantity'], $data['subtract']);
				return array('msg' => 'ok', 'error' => false);
			}
		}else{
			//if not, is there an old link?
			$old_item_id = $this->getEndedEbayItemId($case_id);
			$this->log('caseUpdateListen() - Got item: '.$old_item_id);
			if($old_item_id != false) {
				//yes, check if its a multi variant listing
				if ($this->openbay->addonLoad('openstock') && (isset($data['has_option']) && $data['has_option'] == 1)) {
					//yes, mutli variant listing
					$this->log('caseUpdateListen() - multi variant items relist not supported');
				}else{
					$this->log('caseUpdateListen() - Normal item, checking stock('.$data['quantity'].') > 0');
					//no, its a normal item, is there now stock?
					if($data['quantity'] > 0) {
						//yes, is relist setting yes?
						if($this->config->get('openbaypro_relistitems') == 1) {
							//relist item with new stock
							$this->relistItem($old_item_id, $case_id, $data['quantity']);
						}
					}
				}
			}else{
				//no - list has never existed
				$this->log('caseUpdateListen() - no active or previous item ids');
			}
		}
	}
	...
	public function decideEbayStockAction($case_id, $qty, $subtract, $sku = null) {
		if($subtract == 1) {
			$this->log('decideEbayStockAction() - Case ID: '.$case_id.', Current stock: '.$qty);
			...
			$this->log('decideEbayStockAction() - Case ID: '.$case_id.' does not subtract stock');
		}
	}

	public function getCaseId($ebay_item, $status = 0) {
		$this->log('getCaseId() - Item: '.$ebay_item);
		...
		$qry = $this->db->query("SELECT `case_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `ebay_item_id` = '".$this->db->escape($ebay_item)."'".$status_sql." LIMIT 1");

		if(!$qry->num_rows) {
			return false;
		}else{
			return $qry->row['case_id'];
		}
	}

	public function getCaseIdFromKey($key) {
		$qry = $this->db->query("SELECT `case_id` FROM `" . DB_PREFIX . "ebay_listing_pending` WHERE `key` = '".$this->db->escape($key)."' LIMIT 1");
	...
	public function getAllocatedStock($case_id) {
		$qry = $this->db->query("SELECT SUM(`qty`) AS `total` FROM `" . DB_PREFIX . "ebay_transaction` WHERE `case_id` = '".(int)$case_id."' AND `order_id` = '0' LIMIT 1");
		return (int)$qry->row['total'];
	}

	public function getImages() {
		$this->log('getImages() - Getting case images.');
		...
		if($qry->num_rows) {
			foreach ($qry->rows as $img) {
				...
				if($img['imgcount'] == 0) {
					$this->db->query("UPDATE `" . DB_PREFIX . "case` SET `image` = 'data/".$this->db->escape($img['name'])."' WHERE `case_id` = '".(int)$img['case_id']."' LIMIT 1");
				}else{
					$this->db->query("INSERT INTO `" . DB_PREFIX . "case_image` SET `case_id` = '".(int)$img['case_id']."', `image` = 'data/".$this->db->escape($img['name'])."', `sort_order` = '".(int)$img['imgcount']."'");
				}
	...
	public function relistItem($itemId, $caseId, $qty) {
		$this->log('relistItem() - Starting relist item, ID: '.$itemId.', case: '.$caseId.', qty: '.$qty);
		...
		if(!empty($response['ItemID'])) {
			...
			$this->createLink($caseId, $response['ItemID'], '');
	...
	public function createLink($case_id, $item_id, $variant) {
		$this->deleteCase($case_id);
		$this->removeItemByItemId($item_id);

		$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_listing` SET `case_id` = '".(int)$case_id."', `ebay_item_id` = '".$this->db->escape($item_id)."', `variant` = '".(int)$variant."', `status` = '1'");
	}

	public function addReserve($data, $item_id, $variant) {
		if($variant == 1) {
			foreach($data['opt'] as $variation) {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case_option_relation` WHERE `case_id` = '".(int)$data['case_id']."' AND `var` = '".$this->db->escape($variation['sku'])."' LIMIT 1");

				if($query->row['stock'] != $variation['qty']) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_stock_reserve` SET `case_id` = '".(int)$data['case_id']."', `item_id` = '".$this->db->escape($item_id)."', `variant_id` = '".$this->db->escape($variation['sku'])."', `reserve` = '".(int)$variation['qty']."'");
				}
			}
		}else{
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case` WHERE `case_id` = '".(int)$data['case_id']."' LIMIT 1");

			if($query->row['quantity'] != $data['qty'][0]) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_stock_reserve` SET `case_id`    = '".(int)$data['case_id']."', `item_id` = '".$this->db->escape($item_id)."', `variant_id` = '', `reserve` = '".(int)$data['qty'][0]."'");
			}
		}
	}

	public function getReserve($case_id, $item_id, $sku = '') {
		$this->log('getReserve()');
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `case_id` = '".(int)$case_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
	...
	public function updateReserve($case_id, $item_id, $reserve, $sku = '', $variant = 0) {

		$this->log('updateReserve() - start');
		$this->log('updateReserve() - $case_id: '.$case_id);
		...
		if($reserve == 0) {
			$this->deleteReserve($case_id, $item_id, $sku);
		}else{
			if($this->getReserve($case_id, $item_id, $sku) != false) {
				$this->db->query("UPDATE `" . DB_PREFIX . "ebay_stock_reserve` SET `reserve` = '".(int)$reserve."' WHERE `case_id` = '".(int)$case_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
			}else{
				if($variant == 0) {
					$this->log('updateReserve() - not a variant');
					$this->addReserve(array('case_id' => $case_id, 'qty' => array(0 => $reserve)), $item_id, 0);
				}else{
					$this->log('updateReserve() - variant');
					$this->addReserve(array('case_id' => $case_id, 'opt' => array(array('sku' => $sku, 'qty' => $reserve))), $item_id, 1);
				}
			}
		}
	}

	public function deleteReserve($case_id, $item_id, $sku = '') {
		$this->log('deleteReserve()');
		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `case_id` = '".(int)$case_id."' AND `variant_id` = '".$this->db->escape($sku)."' AND `item_id` = '".$this->db->escape($item_id)."'  LIMIT 1");
	}
