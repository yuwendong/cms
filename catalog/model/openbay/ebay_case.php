...
class ModelOpenbayEbayCase extends Model {
	...
	public function importItems($data) {
		...
		foreach($newData1 as $item) {
			if(!in_array($item['ItemID'], $current)) {
				...
				$this->db->query("
					INSERT INTO `" . DB_PREFIX . "case` SET
				...
				$this->openbay->ebay->log('Case insert done');
				...
				$sql = " INSERT INTO `" . DB_PREFIX . "case_description` SET
						`case_id`            = '".(int)$case_id."',
				...
				$this->db->query($sql);
				$this->openbay->ebay->log('Case description done');

				//Insert case store link
				$this->db->query("INSERT INTO `" . DB_PREFIX . "case_to_store` SET `case_id` = '".(int)$case_id."', `store_id` = '0'");
				...
				if(!empty($item['specs'])) {
					...
					foreach($item['specs'] as $spec) {
						...
						//insert the attribute value into the case attribute table
						$this->attributeAdd($case_id, $attrId, base64_decode($spec['value']));
					}
				}

				//Create the case variants for OpenStock
				$variant = 0;
				if(!empty($item['variation'])) {
					...
					if($openstock == true) {
						...
						$this->createVariants($case_id, $item);
					}
				...
				$this->openbay->ebay->createLink($case_id, $item['ItemID'], $variant);

				//Insert case/category link
				if ($options['cat'] == 1) {
					$this->createCategoryLink($case_id, $catLink[$item['CategoryName']]);
				}
				...
				if(is_array($item['pictures'])) {
					foreach($item['pictures'] as $img) {
						if(!empty($img)) {
							...
							$this->addImage($img, DIR_IMAGE.'data/'.$name.'.jpg', $name.'.jpg', $case_id, $imgCount);
							$imgCount++;
						}
					}
				}

				$this->openbay->ebay->log('Case import completed.');
			}else{
				$this->openbay->ebay->log($item['ItemID'].' exists already');
			}
		}

		$this->openbay->ebay->log('Case data import done');
		$this->openbay->ebay->getImages();
	}

	public function getDisplayCases() {
		$data = array();
	...
	private function createVariants($case_id, $data) {
		foreach($data['variation']['vars'] as $variant) {
			...
			foreach($variant['opt'] as $k_opt => $v_opt) {
				...
				//lookup case option rel table, insert if needed
				$case_option_id          = $this->getCaseOption($case_id, $option['id']);
				$case_option_value_id    = $this->getCaseOptionValue($case_id, $option['id'], $opt['id'], $case_option_id);

				$this->openbay->ebay->log('Option data: '.serialize($option));

				$s          = $option['sort'];
				$vars[$s]   = $case_option_value_id;
			}
			...
			//create the variant
			$this->createCaseVariant(array('var' => $vars, 'price' => $variant['price'], 'stock' => $variant['qty'], 'case_id' => $case_id, 'sku' => $variant['sku']));
		}

		$this->updateVariantListing($case_id, $data['ItemID']);
	...
	private function getCaseOption($case_id, $option_id) {
		$qry = $this->db->query("SELECT * FROM  " . DB_PREFIX . "case_option WHERE case_id = '" . (int)$case_id . "' AND option_id = '" . (int)$option_id . "' LIMIT 1");

		if($qry->num_rows) {
			return $qry->row['case_option_id'];
		}else{
			$this->db->query("INSERT INTO " . DB_PREFIX . "case_option SET case_id = '" . (int)$case_id . "', option_id = '" . (int)$option_id . "', required = '1'");
			return $this->db->getLastId();
		}
	}

	private function getCaseOptionValue($case_id, $option_id, $option_value_id, $case_option_id) {
		$qry = $this->db->query("
			SELECT *
			FROM  `" . DB_PREFIX . "case_option_value`
				WHERE `case_id` = '" . (int)$case_id . "'
				AND `option_id` = '" . (int)$option_id . "'
				AND `case_option_id` = '".(int)$case_option_id."'
				AND `option_value_id` = '".(int)$option_value_id."'
				LIMIT 1
			");

		if($qry->num_rows) {
			return $qry->row['case_option_value_id'];
		}else{
			$this->db->query("
				INSERT INTO " . DB_PREFIX . "case_option_value
				SET
					case_option_id = '" . (int)$case_option_id . "',
					case_id = '" . (int)$case_id . "',
	...
	private function createCaseVariant($data) {
		$this->db->query("
			INSERT INTO `" . DB_PREFIX . "case_option_relation`
			SET
				`case_id`    = '".$this->db->escape($data['case_id'])."',
	...
	private function updateVariantListing($case_id, $item_id) {
		$varData = array();

		$variants           = $this->model_openstock_openstock->getCaseOptionStocks($case_id);
		$groups             = $this->model_catalog_case->getCaseOptions($case_id);
	...
	private function attributeAdd($case_id, $attrId, $name) {
		$this->openbay->ebay->log('Adding case attribute');
		$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case_attribute` WHERE `case_id` = '".(int)$case_id."' AND `attribute_id` = '".(int)$attrId."' AND `language_id` = '".(int)$this->config->get('config_language_id')."'");

		if($sql->num_rows == 0) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "case_attribute` SET `case_id` = '".(int)$case_id."', `attribute_id` = '".(int)$attrId."', `text` = '".$this->db->escape(htmlspecialchars($name, ENT_COMPAT))."', `language_id` = '".(int)$this->config->get('config_language_id')."'");
		}
	}

	private function createCategoryLink($case_id, $category_id) {
	...
	private function addImage($orig, $new, $name, $case_id, $imgCount) {
