...
class ModelOpenbayAmazonOrder extends Model {
  ...
  <MerchantOrderId>$orderId</MerchantOrderId>
	...
	public function getCaseId($sku) {
		$row = $this->db->query("SELECT `case_id`
			FROM `" . DB_PREFIX . "amazon_case_link`
			WHERE `amazon_sku` = '" . $this->db->escape($sku) . "'
			")->row;

		if (isset($row['case_id']) && !empty($row['case_id'])) {
			return $row['case_id'];
		}
	...
	public function getCaseVar($sku) {
	...
	public function decreaseCaseQuantity($caseId, $delta, $var = '') {
		if($caseId == 0) {
			return;
		}
		if($var == '') {
			$this->db->query("
				UPDATE `" . DB_PREFIX . "case`
				SET `quantity` = GREATEST(`quantity` - '" . (int)$delta . "', 0)
				WHERE `case_id` = '" . (int)$caseId . "'");
		} else {
			//@TODO: do something about subtract column?
			$this->db->query("
				UPDATE `" . DB_PREFIX . "case_option_relation`
				SET `stock` = GREATEST(`stock` - '" . (int)$delta . "', 0)
				WHERE `case_id` = '" . (int)$caseId . "' AND `var` = '" . $this->db->escape($var) . "'");
		}
	}
	...
	public function addAmazonOrderCases($orderId, $data) {
		foreach ($data as $sku => $orderItemId) {

			$row = $this->db->query("
				SELECT `order_case_id`
				FROM `" . DB_PREFIX . "order_case`
				WHERE `model` = '" . $this->db->escape($sku) . "' AND `order_id` = " . (int)$orderId . "
				LIMIT 1")->row;

			if (!isset($row['order_case_id']) || empty($row['order_case_id'])) {
				continue;
			}

			$orderCaseId = $row['order_case_id'];

			$this->db->query("
				INSERT INTO `" . DB_PREFIX . "amazon_order_case` (`order_case_id`, `amazon_order_item_id`)
				VALUES (" . (int)$orderCaseId . ", '" . $this->db->escape($orderItemId) . "')");
		}
	}
	...
	public function getCaseOptionsByVar($caseVar) {
		$options = array();

		$optionValueIds = explode(':', $caseVar);
		foreach($optionValueIds as $optionValueId) {
			$optionDetailsRow = $this->db->query("SELECT
				pov.case_option_id,
				pov.case_option_value_id,
				od.name,
				ovd.name as value,
				opt.type
			FROM `" . DB_PREFIX . "case_option_value` as pov,
				 `" . DB_PREFIX . "case_option` as po,
				 `" . DB_PREFIX . "option` as opt,
				 `" . DB_PREFIX . "option_value_description` as ovd,
				 `" . DB_PREFIX . "option_description` as od
			WHERE pov.case_option_value_id = '" . (int)$optionValueId . "' AND
				po.case_option_id = pov.case_option_id AND
			...
			if(!empty($optionDetailsRow)) {
				$options[] = array(
					'case_option_id' => (int)$optionDetailsRow['case_option_id'],
					'case_option_value_id' => (int)$optionDetailsRow['case_option_value_id'],
