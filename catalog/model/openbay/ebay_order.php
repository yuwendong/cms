...
class ModelOpenbayEbayOrder extends Model{
	public function addOrderLine($data, $order_id, $created) {
		...
		if($orderLine === false) {
			if($created >= $from) {
				...
				$case_id = $this->openbay->ebay->getCaseId($data['item_id']);
				...
				if(!empty($case_id)) {
					...
					$this->modifyStock($case_id, $data['qty'], '-', $data['sku']);
				}
			}else{
	...
	private function removeOrderLine($txn_id, $item_id, $line) {
		...
		if($this->db->countAffected() > 0) {
			$this->modifyStock($line['case_id'], $line['qty'], '+', $line['sku']);
		}
	}
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if(isset($order_info['email']) && !empty($order_info['email'])){
				...
				foreach ($order_case_query->rows as $case) {
					...
					$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_case_id = '" . $this->db->escape($case['order_case_id']) . "'");
	...
	private function modifyStock($case_id, $qty, $symbol = '-', $sku = '') {
		$this->openbay->ebay->log('modifyStock() - Updating stock. Case id: '.$case_id.' qty: '.$qty.', symbol: '.$symbol.' sku: '.$sku);

		$item_id = $this->openbay->ebay->getEbayItemId($case_id);

		if ($this->openbay->addonLoad('openstock') && !empty($sku)) {
			$this->db->query("UPDATE `" . DB_PREFIX . "case_option_relation` SET `stock` = (`stock` ".(string)$symbol." " . (int)$qty . ") WHERE `var` = '" . (string)$sku . "' AND `case_id` = '".(int)$case_id."' AND `subtract` = '1'");

			$stock = $this->openbay->ebay->getCaseStockLevel($case_id, $sku);
			...
			$this->db->query("UPDATE `" . DB_PREFIX . "case` SET `quantity` = (`quantity` ".(string)$symbol." " . (int)$qty . ") WHERE `case_id` = '" . (int)$case_id . "' AND `subtract` = '1'");

			$stock = $this->openbay->ebay->getCaseStockLevel($case_id);

			$this->openbay->ebay->log('modifyStock() - Stock is now set to: '.$stock['quantity']);

			//send back stock update to eBay incase of a reserve case level
