<?php
class ModelOpenbayAmazonCase extends Model {
	...
	public function linkItems(array $data) {
		foreach($data as $amazonSku => $caseId) {
			$varRow = $this->db->query("SELECT `var` FROM `" . DB_PREFIX . "amazon_case`
				WHERE `sku` = '" . $amazonSku . "' AND `case_id` = '" . (int)$caseId . "'")->row;
			$var = isset($varRow['var']) ? $varRow['var'] : '';
			$this->linkCase($amazonSku, $caseId, $var);
		}
	}

	public function insertError($data) {
		$this->db->query("
			INSERT INTO `" . DB_PREFIX . "amazon_case_error`
	...
	public function deleteErrors($insertionId) {
		 $this->db->query("DELETE FROM `" . DB_PREFIX . "amazon_case_error` WHERE `insertion_id` = '" . $this->db->escape($insertionId) . "'");
	 }
	...
	public function updateSearch($results) {
		foreach ($results as $result) {
			...
			$this->db->query("
				UPDATE " . DB_PREFIX . "amazon_case_search
				SET matches = " . (int)$resultsFound . ",
					`data` = '" . $this->db->escape($data) . "',
					`status` = 'finished'
				WHERE case_id = " . (int)$result['case_id'] . " AND
	...
	public function addListingReport($data) {
		...
		foreach ($data as $case) {
			$sqlValues[] = " ('" . $this->db->escape($case['marketplace']) . "', '" . $this->db->escape($case['sku']) . "', " . (int)$case['quantity'] . ", '" . $this->db->escape($case['asin']) . "', " . (double) $case['price'] . ") ";
		}
