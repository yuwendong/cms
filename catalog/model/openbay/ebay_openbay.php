...
class ModelOpenbayEbayOpenbay extends Model{
	...
	private function create($order) {
		...
		foreach ($order->txn as $txn) {
			$case_id = $this->openbay->ebay->getCaseId($txn->item->id);

			if($case_id != false){
				$this->openbay->ebay->log('create() - Case ID: "'.$case_id.'" from ebay item: '.$txn->item->id.' was returned');

				if(!empty($txn->item->variantsku) && $openstock == true){
					$model_number = $this->openbay->getCaseModelNumber($case_id, $txn->item->variantsku);
				}else{
					$model_number = $this->openbay->getCaseModelNumber($case_id);
				}
			}else{
				$this->openbay->ebay->log('create() - No case ID from ebay item: '.$txn->item->id.' was returned');
				$model_number = '';
			}
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_case` SET
			...
			$this->openbay->ebay->log('create() - Added order case id '.$order_case_id);
				...
				if(!empty($txn->item->variantsku)){
					...
					if($case_id != false){
						...
						foreach($skuParts as $part){
							$sql = "SELECT
									`pv`.`case_option_id`,
									`pv`.`case_option_value_id`,
									`od`.`name`,
									`ovd`.`name` as `value`,
									`o`.`option_id`,
									`o`.`type`
									FROM `" . DB_PREFIX . "case_option_value` `pv`
									...
									WHERE `pv`.`case_option_value_id` = '".(int)$part."'
									AND `pv`.`case_id` = '".(int)$case_id."'";
							$option_qry = $this->db->query($sql);

							if(!empty($option_qry->row)){
								$pOptions[] = array(
									'case_option_id'         => $option_qry->row['case_option_id'],
									'case_option_value_id'   => $option_qry->row['case_option_value_id'],
						...
						foreach ($pOptions as $option) {
							$this->db->query("
								...
								SET
								`order_id`                  = '" . (int)$order_id . "',
								`order_case_id`          = '" . (int)$order_case_id . "',
								`case_option_id`         = '" . (int)$option['case_option_id'] . "',
								`case_option_value_id`   = '" . (int)$option['case_option_value_id'] . "',
	...
	private function externalApplicationNotify($order_id) {
		...
		/* @todo */
		/* improve this to update when cases are subtracted, NOT just when they are paid */
