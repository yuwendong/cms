...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			// Cases
			$coupon_case_data = array();

			$coupon_case_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_case` WHERE coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");

			foreach ($coupon_case_query->rows as $case) {
				$coupon_case_data[] = $case['case_id'];
			}
			...
			if ($coupon_case_data || $coupon_category_data) {
				foreach ($this->cart->getCases() as $case) {
					if (in_array($case['case_id'], $coupon_case_data)) {
					...
					foreach ($coupon_category_data as $category_id) {
						$coupon_category_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "case_to_category` WHERE `case_id` = '" . (int)$case['case_id'] . "' AND category_id = '" . (int)$category_id . "'");
