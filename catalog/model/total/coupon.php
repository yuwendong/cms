			...
			if ($coupon_info) {
				...
				if (!$coupon_info['case']) {
					...
					foreach ($this->cart->getCases() as $case) {
						if (in_array($case['case_id'], $coupon_info['case'])) {
							$sub_total += $case['total'];
						}
					}					
				}
				...
				foreach ($this->cart->getCases() as $case) {
					...
					if ($status) {
						if ($coupon_info['type'] == 'F') {
							$discount = $coupon_info['discount'] * ($case['total'] / $sub_total);
						} elseif ($coupon_info['type'] == 'P') {
							$discount = $case['total'] / 100 * $coupon_info['discount'];
						}

						if ($case['tax_class_id']) {
							$tax_rates = $this->tax->getRates($case['total'] - ($case['total'] - $discount), $case['tax_class_id']);
