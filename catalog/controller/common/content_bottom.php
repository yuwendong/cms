...
class ControllerCommonContentBottom extends Controller {
	protected function index() {
		...
		if ($route == 'case/category' && isset($this->request->get['path'])) {
			$path = explode('_', (string)$this->request->get['path']);

			$layout_id = $this->model_catalog_category->getCategoryLayoutId(end($path));			
		}

		if ($route == 'case/case' && isset($this->request->get['case_id'])) {
			$layout_id = $this->model_catalog_case->getCaseLayoutId($this->request->get['case_id']);
		}
