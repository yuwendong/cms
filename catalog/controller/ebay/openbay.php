...
class ControllerEbayOpenbay extends Controller {
		...
		if(empty($encrypted)) {
			...
			if($secret == $data['secret'] && $active == 1) {
				if($data['action'] == 'ItemUnsold') {
					...
					if($case_id != false) {
						$this->openbay->ebay->log('eBay item link found with internal case');
						$rules = $this->model_openbay_ebay_case->getRelistRule($data['itemId']);

						if(!empty($rules)) {
							$this->openbay->ebay->log('Item is due to be automatically relisted');
							$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_listing_pending` SET `ebay_item_id` = '".$this->db->escape($data['itemId'])."', `case_id` = '".(int)$case_id."', `key` = '".$this->db->escape($data['key'])."'");
				...
				if($data['action'] == 'ItemListed') {
					...
					$case_id = $this->openbay->ebay->getCaseIdFromKey($data['key']);

					if($case_id != false) {
						$this->openbay->ebay->createLink($case_id, $data['itemId'], '');
						$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_listing_pending` WHERE `key` = '".$data['key']."' LIMIT 1");
						$this->openbay->ebay->log('A link was found with case id: '.$case_id.', item id: '.$data['itemId'].' and key: '.$data['key']);
					} else {
	...
	public function importItems() {
		...
		if(isset($data['secret']) && $secret == $data['secret'] && $active == 1 && isset($data['data'])) {
			$this->load->model('openbay/ebay_openbay');
			$this->load->model('openbay/ebay_case');
			$this->model_openbay_ebay_case->importItems($data);
