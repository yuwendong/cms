...
class ControllerAmazonusOrder extends Controller {
	public function index() {
		...
		foreach ($orderXml->Items->Item as $item) {
			...
			$casesTotal += $totalPrice;
			$casesTax += $taxTotal;

			$casesShipping += $this->currency->convert((double)$item->Totals->Shipping, $orderCurrency, $currencyTo);
			...
			$casesShippingTax += $this->currency->convert($shippingTax, $orderCurrency, $currencyTo);
			...
			$caseCount += (int)$item->Ordered;

			if ((int)$item->Ordered == 0) {
				continue;
			}

			$caseId = $this->model_openbay_amazonus_order->getCaseId((string)$item->Sku);
			$caseVar = $this->model_openbay_amazonus_order->getCaseVar((string)$item->Sku);
			...
			$cases[] = array(
				...
				'reward' => '0',
				'option' => $this->model_openbay_amazonus_order->getCaseOptionsByVar($caseVar),
				'download' => array(),
			);

			$caseMapping[(string)$item->Sku] = (string)$item->OrderItemId;
		}
		...
		$order = array(
			...
			'totals' => array(
				array(
					...
					'text' => $this->currency->format($casesTotal, $orderCurrency),
					'value' => sprintf('%.4f', $casesTotal),
					'sort_order' => '1',
				),
				array(
					'code' => 'shipping',
					'title' => $this->language->get('shipping_text'),
					'text' => $this->currency->format($casesShipping, $orderCurrency),
					'value' => sprintf('%.4f', $casesShipping),
					'sort_order' => '3',
				),
				array(
					'code' => 'tax',
					'title' => $this->language->get('tax_text'),
					'text' => $this->currency->format($casesTax, $orderCurrency),
					'value' => sprintf('%.4f', $casesTax),
					'sort_order' => '4',
				),
				array(
					'code' => 'shipping_tax',
					'title' => $this->language->get('shipping_tax_text'),
					'text' => $this->currency->format($casesShippingTax, $orderCurrency),
					'value' => sprintf('%.4f', $casesShippingTax),
					'sort_order' => '6',
				),
				array(
					'code' => 'gift_wrap',
		...
		$this->model_openbay_amazonus_order->addAmazonusOrderCases($orderId, $caseMapping);

		foreach($cases as $case) {
			if($case['case_id'] != 0) {
				$this->model_openbay_amazonus_order->decreaseCaseQuantity($case['case_id'], $case['quantity'], $case['var']);
			}
		}
