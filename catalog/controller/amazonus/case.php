<?php
class ControllerAmazonusCase extends Controller  {
	public function inbound() {
		...
		$logger->write("AmazonusCase/inbound: incoming data");
		...
		if($status == "submit_error") {
			$message = 'Case was not submited to amazonus properly. Please try again or contact OpenBay.';
			$this->model_openbay_amazonus_case->setSubmitError($decodedData['insertion_id'], $message);
		} else {
			$status = (array)$status;
			if($status['successful'] == 1) {
				$this->model_openbay_amazonus_case->setStatus($decodedData['insertion_id'], 'ok');
				$insertionCase = $this->model_openbay_amazonus_case->getCase($decodedData['insertion_id']);
				$this->model_openbay_amazonus_case->linkCase($insertionCase['sku'], $insertionCase['case_id'], $insertionCase['var']);
				$this->model_openbay_amazonus_case->deleteErrors($decodedData['insertion_id']);

				$quantityData = array(
					$insertionCase['sku'] => $this->model_openbay_amazonus_case->getCaseQuantity($insertionCase['case_id'], $insertionCase['var'])
				);
				...
				$msg = 'Case was not accepted by amazonus. Please try again or contact OpenBay.';
				$this->model_openbay_amazonus_case->setSubmitError($decodedData['insertion_id'], $msg);
				...
				if(isset($decodedData['error_details'])) {
					foreach($decodedData['error_details'] as $error) {
						...
						$this->model_openbay_amazonus_case->insertError($error_data);
