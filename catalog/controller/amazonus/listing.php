...
class ControllerAmazonusListing extends Controller {
	public function index() {
		...
		if ($data['status']) {
			$logger->write("Updating " . $data['case_id'] . ' as successful');
			$this->model_openbay_amazonus_listing->listingSuccessful($data['case_id']);
			$this->model_openbay_amazonus_case->linkCase($data['sku'], $data['case_id']);
			$logger->write("Updated successfully");
		} else {
			$logger->write("Updating " . $data['case_id'] . ' as failed');
			$this->model_openbay_amazonus_listing->listingFailed($data['case_id'], $data['messages']);
			$logger->write("Updated successfully");
		}
	}
}
