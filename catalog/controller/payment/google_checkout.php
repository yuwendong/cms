...
class ControllerPaymentGoogleCheckout extends Controller {
	...
	public function send() {
		...
		//	$xml .= '   	</merchant-private-data>'; 
			...
			foreach ($cases as $case) { 
				$xml .= '			<item>';
				$xml .= '				<merchant-item-id>' . $case['key'] . '</merchant-item-id>';
				...
				if ($option_data) {
					$xml .= '				<item-name>' . $case['name'] . ' ' . implode('; ', $option_data) . '</item-name>'; 
					$xml .= '				<item-description>' . $case['name'] . ' ' . implode('; ', $option_data) . '</item-description>';  
				} else {
					$xml .= '				<item-name>' . $case['name'] . '</item-name>'; 
					$xml .= '				<item-description>' . $case['name'] . '</item-description>';  
				}

				$xml .= '				<unit-price currency="' . $this->currency->getCode() . '">' . $this->currency->format($case['price'], $this->currency->getCode(), false, false) . '</unit-price>';
				$xml .= '				<quantity>' . $case['quantity'] . '</quantity>';
				$xml .= '			</item>'; 
			}
  ...
  &shopping-cart.items.item-2.merchant-private-item-data=merchant-case-id%3D1234567890
