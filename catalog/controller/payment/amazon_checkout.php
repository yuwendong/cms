...
class ControllerPaymentAmazonCheckout extends Controller {
	public function address() {
		...
		// CBA supports up to 50 distinct cases
		if (count($this->cart->getCases()) > 50) {
	...
	public function confirm() {
		...
		if (!empty($this->session->data['vouchers']) || !$this->cart->hasCases() || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
		...
		foreach ($this->cart->getCases() as $case) {
			...
			foreach ($case['option'] as $option) {
				...
				$option_data[] = array(
					'case_option_id' => (isset($option['case_option_id']) ? $option['case_option_id'] : ''),
			...
			$case_tax = $this->tax->getTax($case['price'], $case['tax_class_id']);

			if (isset($case['reward'])) {
				$reward = $case['reward'];
			} else {
				$reward = '';
			}

			if (isset($case['subtract'])) {
	...
	public function process_order() {
		...
		$ordered_cases = $this->model_account_order->getOrderCases($order['order_id']);
		...
		foreach ($ordered_cases as $case) {

			$parameters_items['cases'][] = array(
				'title' => html_entity_decode($case['name'], ENT_QUOTES, 'UTF-8'),
				'model' => $case['order_case_id'],
				'quantity' => $case['quantity'],
				'price' => $this->currency->format($case['price'] + $case['tax'], $currency_code, '', false),
			);

			$total += ($case['price'] + $case['tax']) * $case['quantity'];
