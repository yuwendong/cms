...
class ControllerPaymentPPExpress extends Controller {
	...
	public function express() {
		if ((!$this->cart->hasCases() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
		...
		if($this->customer->isLogged()) {
			...
			if($this->config->get('config_guest_checkout') && !$this->config->get('config_customer_price') && !$this->cart->hasDownload() && !$this->cart->hasRecurringCases()) {
				/**
	...
	public function expressComplete() {
		...
		// Validate cart has cases and has stock.
		if ((!$this->cart->hasCases() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
		...
		if ($redirect == '') {
			...
			foreach ($this->cart->getCases() as $case) {
				...
				foreach ($case['option'] as $option) {
					...
					$option_data[] = array(
						...
						'case_option_value_id' => $option['case_option_value_id'],
				...
				$case_data[] = array(
					...
					'quantity' => $case['quantity'],
					'subtract' => $case['subtract'],
					'price' => $case['price'],
					'total' => $case['total'],
					'tax' => $this->tax->getTax($case['price'], $case['tax_class_id']),
					'reward' => $case['reward']
				);
			}
			...
			if($result['ACK'] == 'Success') {
				...
				$recurring_cases = $this->cart->getRecurringCases();

				//loop through any cases that are recurring items
				if(!empty($recurring_cases)) {
					...
					foreach($recurring_cases as $item) {
						$data = array(
