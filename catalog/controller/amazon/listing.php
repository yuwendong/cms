...
class ControllerAmazonListing extends Controller {
	public function index() {
		...
		if ($data['status']) {
			$logger->write("Updating " . $data['case_id'] . ' from ' . $data['marketplace'] . ' as successful');
			$this->model_openbay_amazon_listing->listingSuccessful($data['case_id'], $data['marketplace']);
			$this->model_openbay_amazon_case->linkCase($data['sku'], $data['case_id']);
			$logger->write("Updated successfully");
		} else {
			$logger->write("Updating " . $data['case_id'] . ' from ' . $data['marketplace'] . ' as failed');
			$this->model_openbay_amazon_listing->listingFailed($data['case_id'], $data['marketplace'], $data['messages']);
			$logger->write("Updated successfully");
		}
	}
}
