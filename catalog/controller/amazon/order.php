...
class ControllerAmazonOrder extends Controller {
	public function index() {
		...
		foreach ($orderXml->Items->Item as $item) {
			...
			$caseId = $this->model_openbay_amazon_order->getCaseId((string)$item->Sku);
			$caseVar = $this->model_openbay_amazon_order->getCaseVar((string)$item->Sku);
			...
			$cases[] = array(
				...
				'reward' => '0',
				'option' => $this->model_openbay_amazon_order->getCaseOptionsByVar($caseVar),
				'download' => array(),
			);
		...
		$this->model_openbay_amazon_order->addAmazonOrderCases($orderId, $caseMapping);

		foreach($cases as $case) {
			if($case['case_id'] != 0) {
				$this->model_openbay_amazon_order->decreaseCaseQuantity($case['case_id'], $case['quantity'], $case['var']);
			}
		}
