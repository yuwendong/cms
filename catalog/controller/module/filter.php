...
class ControllerModuleFilter extends Controller {
	protected function index($setting) {
		...
		if ($category_info) {
			...
			$this->data['action'] = str_replace('&amp;', '&', $this->url->link('case/category', 'path=' . $this->request->get['path'] . $url));
				...
				foreach ($filter_groups as $filter_group) {
					...
					foreach ($filter_group['filter'] as $filter) {
						...
						$filter_data[] = array(
							...
							'name'      => $filter['name'] . ($this->config->get('config_case_count') ? ' (' . $this->model_catalog_case->getTotalCases($data) . ')' : '')
						);
					}
