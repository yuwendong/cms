...
class ControllerModuleAmazonCheckoutLayout extends Controller {
	protected function index($setting) {
		if ($this->config->get('amazon_checkout_status') == 1 && $setting['status']) {
			...
			if ((empty($allowed_ips) || in_array($this->request->server['REMOTE_ADDR'], $allowed_ips)) && $this->cart->hasCases()
					&& (!isset($this->session->data['vouchers']) || empty($this->session->data['vouchers'])) && !$this->cart->hasRecurringCases()) {
