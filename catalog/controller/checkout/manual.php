	...
	public function index() {
		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {	
				...
				foreach ($this->request->post['order_case'] as $order_case) {
					...
					if ($case_info) {	
						...
						if (isset($order_case['order_option'])) {
							foreach ($order_case['order_option'] as $option) {
								if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'image') { 
									$option_data[$option['case_option_id']] = $option['case_option_value_id'];
								} elseif ($option['type'] == 'checkbox') {
									$option_data[$option['case_option_id']][] = $option['case_option_value_id'];
								} elseif ($option['type'] == 'text' || $option['type'] == 'textarea' || $option['type'] == 'file' || $option['type'] == 'date' || $option['type'] == 'datetime' || $option['type'] == 'time') {
									$option_data[$option['case_option_id']] = $option['value'];						
								}
							}
						}

						$this->cart->add($order_case['case_id'], $order_case['quantity'], $option_data);
					}
				}
			}

			if (isset($this->request->post['case_id'])) {
				$case_info = $this->model_catalog_case->getCase($this->request->post['case_id']);
				...
				if ($case_info) {
					...
					foreach ($case_options as $case_option) {
						if ($case_option['required'] && empty($option[$case_option['case_option_id']])) {
							$json['error']['case']['option'][$case_option['case_option_id']] = sprintf($this->language->get('error_required'), $case_option['name']);
						}
					}

					if (!isset($json['error']['case']['option'])) {
						$this->cart->add($this->request->post['case_id'], $quantity, $option);
					}
				}
			}
			...
			if (!$this->cart->hasStock() && (!$this->config->get('config_stock_checkout') || $this->config->get('config_stock_warning'))) {
				$json['error']['case']['stock'] = $this->language->get('error_stock');
			}		
			...
			foreach ($cases as $case) {
				...
				if ($case['minimum'] > $case_total) {
					$json['error']['case']['minimum'][] = sprintf($this->language->get('error_minimum'), $case['name'], $case['minimum']);
				}	
