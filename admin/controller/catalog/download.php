...
class ControllerCatalogDownload extends Controller {
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $download_id) {
			$case_total = $this->model_catalog_case->getTotalCasesByDownloadId($download_id);

			if ($case_total) {
				$this->error['warning'] = sprintf($this->language->get('error_case'), $case_total);
			}
		}
	...
	public function upload() {
		...
		if (!isset($json['error'])) {
			if (is_uploaded_file($this->request->files['file']['tmp_name']) && file_exists($this->request->files['file']['tmp_name'])) {
				...
				$json['filename'] = $this->request->files['file']['name'] . '.' . $ext;
				$json['mask'] = $this->request->files['file']['name'];

				move_uploaded_file($this->request->files['file']['tmp_name'], DIR_DOWNLOAD . iconv("UTF-8","gb2312",$this->request->files['file']['name']) . '.' . $ext);
			}
