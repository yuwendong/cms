...
class ControllerPaymentAmazonCheckout extends Controller {
	...
	public function orderAction() {
		...
		if ($amazon_order_info) {
			$order_cases = $this->model_sale_order->getOrderCases($order_id);
			$order_info = $this->model_sale_order->getOrder($order_id);

			$this->data['cases'] = array();

			foreach ($order_cases as $case) {
				$case_options = $this->model_sale_order->getOrderOptions($order_id, $case['order_case_id']);

				$order_item_code = '';

				if (isset($amazon_order_info['cases'][$case['order_case_id']])) {
					$order_item_code = $amazon_order_info['cases'][$case['order_case_id']]['amazon_order_item_code'];
				}
				...
				$this->data['cases'][] = array(
					...
					'price' => $this->currency->format($case['price'], $order_info['currency_code'], $order_info['currency_value']),
					'total' => $this->currency->format($case['total'], $order_info['currency_code'], $order_info['currency_value']),
