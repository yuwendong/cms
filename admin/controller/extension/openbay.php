...
class ControllerExtensionOpenbay extends Controller {
	...
	public function linkStatus(){
		...
		if ($this->config->get('openbay_status') == '1') {
			...
			if($this->openbay->ebay->getEbayItemId($case_id) == false) {
				$markets[] = array(
					'name'      => 'eBay',
					'status'    => 0,
					'text'      => $this->language->get('lang_openbay_new'),
					'href'      => $this->url->link('openbay/openbay/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => ''
				);
			} else {
				$markets[] = array(
					...
					'href'      => $this->url->link('openbay/openbay/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => ''
				);
			}
		}
		...
		if ($this->config->get('amazon_status') == '1') {
			...
			$amazon_status = $this->model_openbay_amazon->getCaseStatus($case_id);
			...
			if ($amazon_status == 'processing') {
				...
				$markets[] = array(
					...
					'status' => 1,
					'text' => $this->language->get('lang_openbay_edit'),
					'href' => $this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => '',
				);
				...
				$markets[] = array(
					...
					'href' => $this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => '',
				);
			} else {
				$markets[] = array(
		...
		if ($this->config->get('amazonus_status') == '1') {
			...
			$amazonus_status = $this->model_openbay_amazonus->getCaseStatus($case_id);
			...
			if ($amazonus_status == 'processing') {
				...
				$markets[] = array(
					...
					'status' => 1,
					'text' => $this->language->get('lang_openbay_edit'),
					'href' => $this->url->link('openbay/amazonus_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => '',
				);
				...
				$markets[] = array(
					...
					'status' => 0,
					'text' => $this->language->get('lang_openbay_fix'),
					'href' => $this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL'),
					'target'    => '',
				);
			} else {
				$markets[] = array(
	...
	public function itemList() {
		...
		$this->data = array_merge($this->data, $this->load->language('catalog/case'));
		$this->data = array_merge($this->data, $this->load->language('extension/default'));
		...
		if ($this->config->get('amazon_status')) {
			$this->data['href_amazon_bulk_list'] = $this->url->link('openbay/amazon/bulkListCases', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
		...
		$case_total = $this->model_openbay_openbay->getTotalCases($data);

		$results = $this->model_openbay_openbay->getCases($data);

		foreach ($results as $result) {
			$edit = $this->url->link('catalog/case/update', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL');
			...
			if ($this->config->get('openbay_status') == '1') {
				...
				if(!array_key_exists($result['case_id'], $activeList)) {
					$markets[] = array(
						'name'      => 'eBay',
						'img'       => HTTPS_SERVER.'view/image/openbay/ebay_nolink.png',
						'text'      => $this->language->get('lang_openbay_new'),
						'href'      => $this->url->link('openbay/openbay/create', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				} else {
					$markets[] = array(
						'name'      => 'eBay',
						...
						'text'      => $this->language->get('lang_openbay_edit'),
						'href'      => $this->url->link('openbay/openbay/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				}
			}
			...
			if ($this->config->get('amazon_status') == '1') {
				$this->load->model('openbay/amazon');
				$amazon_status = $this->model_openbay_amazon->getCaseStatus($result['case_id']);
				...
				if($amazon_status == 'processing') {
					...
					$markets[] = array(
						...
						'text'      => $this->language->get('lang_openbay_edit'),
						'href'      => $this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				} else if ($amazon_status == 'error_quick' || $amazon_status == 'error_advanced' || $amazon_status == 'error_few') {
					$markets[] = array(
						'name'      => 'Amazon EU',
						...
						'text'      => $this->language->get('lang_openbay_fix'),
						'href'      => $this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				} else {
					$markets[] = array(
						'name'      => 'Amazon EU',
			...
			if ($this->config->get('amazonus_status') == '1') {
				$this->load->model('openbay/amazonus');
				$amazonus_status = $this->model_openbay_amazonus->getCaseStatus($result['case_id']);
				...
				if($amazonus_status == 'processing') {
					...
					$markets[] = array(
						...
						'img'       => HTTPS_SERVER.'view/image/openbay/amazon_ok.png',
						'text'      => $this->language->get('lang_openbay_edit'),
						'href'      => $this->url->link('openbay/amazonus_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				} else if ($amazonus_status == 'error_quick' || $amazonus_status == 'error_advanced' || $amazonus_status == 'error_few') {
					$markets[] = array(
						'name'      => 'Amazon US',
						'img'       => HTTPS_SERVER.'view/image/openbay/amazon_error.png',
						'text'      => $this->language->get('lang_openbay_fix'),
						'href'      => $this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'] . $url, 'SSL'),
					);
				} else {
					$markets[] = array(
						'name'      => 'Amazon US',
			...
			$this->data['cases'][] = array(
				...
				'vCount'     => $openstock_installed ? $this->model_openstock_openstock->countVariation($result['case_id']) : '',
				'vsCount'    => $openstock_installed ? $this->model_openstock_openstock->countVariationStock($result['case_id']) : '',
			);
		}
