...
class ControllerOpenbayAmazonListing extends Controller {
	public function create() {
		...
		if ($this->request->post) {
			...
			$result = $this->model_openbay_amazon_listing->simpleListing($case);
			...
			if($result['status'] === 1) {
				...
				$this->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->post['case_id'] . $url, 'SSL'));
			}
		}
		...
		if (isset($this->request->get['case_id'])) {
			...
			$listing_status = $this->model_openbay_amazon->getCaseStatus($this->request->get['case_id']);

			if($listing_status === 'processing' || $listing_status === 'ok') {
				$this->redirect($this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'] . $url, 'SSL'));
			} else if ($listing_status === 'error_advanced' || $listing_status === 'saved' || $listing_status === 'error_few') {
				$this->redirect($this->url->link('openbay/amazon_case', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'] . $url, 'SSL'));
			}
		} else {
			$this->redirect($this->url->link('extension/openbay/itemList', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['url_advanced']  = $this->url->link('openbay/amazon_case', 'token=' . $this->session->data['token'] . '&case_id='.$this->request->get['case_id'] . $url, 'SSL');
		...
		if ($this->config->get('openbay_amazon_listing_tax_added')) {
			$this->data['price'] = $case_info['price'] * (1 + $this->config->get('openbay_amazon_listing_tax_added') / 100);
		} else {
		...
		if ($listing_status == 'error_quick') {
			$this->data['listing_errors'] = $this->model_openbay_amazon->getCaseErrors($case_info['case_id'], 3);
		}
	...
	public function edit() {
		...
		$status = $this->model_openbay_amazon->getCaseStatus($case_id);

		//If case was not submited/saved for Amazon
		if($status === false) {
			$this->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'));
			return;
		}

		$this->data['case_links'] = $this->model_openbay_amazon->getCaseLinks($case_id);
		$this->data['url_return']  = $this->url->link('extension/openbay/itemList', 'token=' . $this->session->data['token'] . $url, 'SSL');
		if($status == 'ok' || $status == 'linked') {
			$this->data['url_create_new']  = $this->url->link('openbay/amazon_listing/createNew', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL');
			$this->data['url_delete_links']  = $this->url->link('openbay/amazon_listing/deleteLinks', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL');
		}
		...
		$this->data['url_saved_listings']  = $this->url->link('openbay/amazon/savedListings', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL');
		...
		$this->data['breadcrumbs'][] = array(
			'href' => $this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'),
	...
	public function createNew() {
		...
		$this->load->model('openbay/amazon');
		$this->model_openbay_amazon->deleteCase($case_id);
		$this->redirect($this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'));
	}
	...
	public function deleteLinks() {
		...
		$links = $this->model_openbay_amazon->getCaseLinks($case_id);
		foreach ($links as $link) {
			$this->model_openbay_amazon->removeCaseLink($link['amazon_sku']);
		}
		$this->model_openbay_amazon->deleteCase($case_id);
	...
	public function getCaseByAsin() {
		...
		$data = $this->model_openbay_amazon_listing->getCaseByAsin($this->request->post['asin'], $this->request->post['market']);
