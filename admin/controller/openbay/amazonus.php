...
class ControllerOpenbayAmazonus extends Controller {
	public function stockUpdates() {
		...
		if($simpleXmlObj !== false) {
			...
			foreach($simpleXmlObj->update as $updateNode) {
				...
				$data = array();
				foreach($updateNode->data->case as $caseNode) {
					$data[] = array('sku' => (string)$caseNode->sku,
						'stock' => (int)$caseNode->stock
						);
				}
				$row['data'] = $data;
	...
	public function overview() {
		...
		$this->data['link_bulk_listing'] = $this->url->link('openbay/amazonus/bulkListCases', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function subscription() {
		...
		if ($response) {
			foreach ($response->Plan as $plan) {
				$plans[] = array(
					...
					'case_listings' => (string)$plan->CaseListings,
		...
		if ($response) {
			$plan = array(
				...
				'case_listings' => (string)$response->CaseListings,
	...
	public function settings() {
		...
		$this->data['itemLinks_url'] = $this->url->link('openbay/amazonus_case/linkItems', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function savedListings() {
		...
		$this->data['token'] = $this->session->data['token'];
		$this->load->model('openbay/amazonus');
		$saved_cases = $this->model_openbay_amazonus->getSavedCases();

		$this->data['saved_cases'] = array();

		foreach($saved_cases as $saved_case) {
			$this->data['saved_cases'][] = array(
				'case_id' => $saved_case['case_id'],
				'case_name' => $saved_case['case_name'],
				'case_model' => $saved_case['case_model'],
				'case_sku' => $saved_case['case_sku'],
				'amazonus_sku' => $saved_case['amazonus_sku'],
				'var' => $saved_case['var'],
				'edit_link' => $this->url->link('openbay/amazonus_case', 'token=' . $this->session->data['token'] . '&case_id=' . $saved_case['case_id'] . '&var=' . $saved_case['var'], 'SSL'),
			);
		}

		$this->data['deleteSavedAjax'] = $this->url->link('openbay/amazonus/deleteSavedAjax', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['uploadSavedAjax'] = $this->url->link('openbay/amazonus_case/uploadSavedAjax', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function getOpenstockOptionsAjax() {
		$options = array();
		if ($this->openbay->addonLoad('openstock') && isset($this->request->get['case_id'])) {
			$this->load->model('openstock/openstock');
			$this->load->model('tool/image');
			$options = $this->model_openstock_openstock->getCaseOptionStocks($this->request->get['case_id']);
		}
		if(empty($options)) {
			$options = false;
		}
	...
	public function addItemLinkAjax() {
		...
		if(isset($this->request->get['case_id']) && isset($this->request->get['amazonus_sku'])) {
		...
		$this->model_openbay_amazonus->linkCase($amazonus_sku, $case_id, $var);
		$logger = new Log('amazonus_stocks.log');
		$logger->write('addItemLink() called for case id: ' . $case_id . ', amazonus sku: ' . $amazonus_sku . ', var: ' . $var);
		...
		if($var != '' && $this->openbay->addonLoad('openstock')) {
			...
			$this->openbay->amazonus->putStockUpdateBulk(array($case_id));
		}
	...
	public function removeItemLinkAjax() {
		...
		$this->model_openbay_amazonus->removeCaseLink($amazonus_sku);
	...
	public function getItemLinksAjax() {
		...
		$itemLinks = $this->model_openbay_amazonus->getCaseLinks();
	...
	public function getUnlinkedItemsAjax() {
		...
		$unlinkedCases = $this->model_openbay_amazonus->getUnlinkedCases();
		$result = json_encode($unlinkedCases);
		$this->response->setOutput($result);
	}

	public function deleteSavedAjax() {
		if(!isset($this->request->get['case_id']) || !isset($this->request->get['var'])) {
			return;
		}

		$this->load->model('openbay/amazonus');
		$this->model_openbay_amazonus->deleteSaved($this->request->get['case_id'], $this->request->get['var']);
	}

	public function doBulkList() {
		$this->load->language('amazonus/listing');

		if (empty($this->request->post['cases'])) {
			$json = array(
			...
			$bulk_list_cases = array();

			foreach ($this->request->post['cases'] as $case_id => $asin) {
				$delete_search_results[] = $case_id;

				if (!empty($asin)) {
					$bulk_list_cases[$case_id] = $asin;
				}
			}
			...
			if ($bulk_list_cases) {
				...
				if ($status) {
					$message = $this->language->get('text_cases_sent');

					if ($delete_search_results) {
						$this->model_openbay_amazonus_listing->deleteSearchResults($delete_search_results);
					}
				} else {
					$message = $this->language->get('error_sending_cases');
				}
			} else {
				$message = $this->language->get('error_no_cases_selected');
			}
	...
	public function doBulkSearch() {
		...
		if (!empty($this->request->post['case_ids'])) {
			foreach ($this->request->post['case_ids'] as $case_id) {
				$case = $this->model_catalog_case->getCase($case_id);

				if (empty($case['sku'])) {
					$json[$case_id] = array(
						'error' => $this->language->get('error_case_sku')
					);
				}
				...
				if (!$key) {
					$json[$case_id] = array(
						'error' => $this->language->get('error_case_no_searchable_fields')
					);
				}

				if (!isset($json[$case_id])) {
					$search_data[$key][] = array(
						'case_id' => $case['case_id'],
						'value' => trim($case[$id_type]),
					);
	...
	public function bulkListCases() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('openbay/amazonus/bulkListCases', 'token=' . $this->session->data['token'], 'SSL'),
		...
		if ($bulk_listing_status) {
			...
			$results = $this->model_openbay_amazonus->getCaseSearch($data);
			$case_total = $this->model_openbay_amazonus->getCaseSearchTotal($data);

			$this->data['cases'] = array();

			foreach ($results as $result) {
				$case = $this->model_catalog_case->getCase($result['case_id']);

				if ($case['image'] && file_exists(DIR_IMAGE . $case['image'])) {
					$image = $this->model_tool_image->resize($case['image'], 40, 40);
				} else {
				...
				$href = $this->url->link('catalog/case/update', 'token=' . $this->session->data['token'] . '&case_id=' . $case['case_id'], 'SSL');
			...
			$pagination->url = $this->url->link('openbay/amazonus/bulkListCases', 'token=' . $this->session->data['token'] . '&page={page}', 'SSL');
	...
	public function bulkLinking() {
		...
		foreach ($results as $result) {
			$cases[] = array(
				...
				'var' => $result['var'],
				'href_case' => $this->url->link('catalog/case/update', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'], 'SSL'),
			);
		}

		$this->data['unlinked_cases'] = $cases;
	...
	public function doBulkLinking() {
		...
		if (!empty($links)) {
			foreach ($links as $link) {
				$this->model_openbay_amazonus->linkCase($link['amazon_sku'], $link['case_id'], $link['var']);
			}
