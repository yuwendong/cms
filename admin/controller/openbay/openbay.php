...
class ControllerOpenbayOpenbay extends Controller {
	...
	public function searchEbayCatalog() {
		...
		$json = $this->model_openbay_ebay_case->searchEbayCatalog($this->request->post);
	...
	public function getCaseStock() {
		...
		$json = $this->model_openbay_ebay->getCaseStock($this->request->get['pid']);

		$this->response->setOutput(json_encode($json));
	}

	public function setCaseStock() {


		$this->load->model('openbay/ebay');
		$this->load->model('catalog/case');

		$case = $this->model_catalog_case->getCase($this->request->get['case_id']);

		$json = array();

		if ($case['subtract'] == 1) {
			$this->openbay->ebay->caseUpdateListen($this->request->get['case_id'], $case);
		...
		if ($this->request->post['pass'] == '') {
			...
			if ($this->request->post['pass'] != $this->config->get('openbaypro_secret')) {
				...
				/*
				  ...
				  $this->db->query("TRUNCATE `" . DB_PREFIX . "order_case`");
				...
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_to_store`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_description`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_attribute`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_option`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_option_value`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_image`");
				$this->db->query("TRUNCATE `" . DB_PREFIX . "case_to_category`");
				...
				if ($this->openbay->addonLoad('openstock')) {
					$this->db->query("TRUNCATE `" . DB_PREFIX . "case_option_relation`");
				}
	...
	public function viewItemImport() {
		...
		$this->data['validation']       = $this->openbay->ebay->validate();
		$this->data['token']            = $this->session->data['token'];
		$this->data['imgImport']        = $this->model_openbay_ebay_case->countImportImages();
	...
	public function viewItemLinks() {
		...
		$this->data['return']       = $this->url->link('openbay/openbay', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['edit_url']     = $this->url->link('openbay/openbay/edit', 'token=' . $this->session->data['token'].'&case_id=', 'SSL');
		$this->data['validation']   = $this->openbay->ebay->validate();
		$this->data['token']        = $this->session->data['token'];
	...
	public function removeItemLink() {
		...
		$this->openbay->ebay->removeItemByCaseId($this->request->get['case_id']);
	...
	public function edit() {
		if ($this->checkConfig() == true) {
			if (!empty($this->request->get['case_id'])) {
				...
				$this->data['view_link']    = $this->config->get('openbaypro_ebay_itm_link') . $this->openbay->ebay->getEbayItemId($this->request->get['case_id']);
				$this->data['token']        = $this->session->data['token'];
				...
				$this->data['breadcrumbs'][] = array(
					'href' => $this->url->link('openbay/openbay/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'], 'SSL'),
	...
	public function editLoad() {
		...
		$item_id        = $this->openbay->ebay->getEbayItemId($this->request->get['case_id']);

		if (!empty($item_id)) {
			$listings   = $this->openbay->ebay->getEbayListing($item_id);
			$stock      = $this->openbay->ebay->getCaseStockLevel($this->request->get['case_id']);
			$reserve    = $this->openbay->ebay->getReserve($this->request->get['case_id'], $item_id);
			$options    = array();

			$case_info = $this->model_catalog_case->getCase($this->request->get['case_id']);

			if ($this->openbay->addonLoad('openstock') && $case_info['has_option'] == 1) {
				$this->load->model('openstock/openstock');
				$this->data['addon']['openstock'] = true;
				$case_info['options'] = $this->model_openstock_openstock->getCaseOptionStocks($this->request->get['case_id']);
				$case_info['option_grp'] = $this->model_openbay_ebay_case->getCaseOptions($this->request->get['case_id']);

				$t = array();
				$t_rel = array();

				foreach($case_info['option_grp'] as $grp) {
					$t_tmp = array();

					foreach($grp['case_option_value'] as $grp_node) {
						$t_tmp[$grp_node['option_value_id']] = $grp_node['name'];
						$t_rel[$grp_node['case_option_value_id']] = $grp['name'];
					}
				...
				foreach($case_info['options'] as $option) {
					$option['base64']   = base64_encode(serialize($option['opts']));
					$optionReserve      = $this->openbay->ebay->getReserve($this->request->get['case_id'], $item_id, $option['var']);
					if($optionReserve == false) {
						$option['reserve'] = 0;
					}else{
						$option['reserve']  = $this->openbay->ebay->getReserve($this->request->get['case_id'], $item_id, $option['var']);
					}
	...
	public function create() {
		if ($this->checkConfig() == true) {
			if (!empty($this->request->get['case_id'])) {
				...
				// get the case tax rate from opencart
				if (isset($case_info['tax_class_id'])) {
					$case_info['defaults']['tax'] = $this->model_openbay_ebay_case->getTaxRate($case_info['tax_class_id']);
				} else {
					$case_info['defaults']['tax'] = 0.00;
				}

				//get the popular categories the user has used
				$case_info['popular_cats'] = $this->model_openbay_ebay->getPopularCategories();

				//get shipping profiles
				$case_info['profiles_shipping'] = $this->model_openbay_ebay_profile->getAll(0);
				//get default shipping profile
				$case_info['profiles_shipping_def'] = $this->model_openbay_ebay_profile->getDefault(0);

				//get returns profiles
				$case_info['profiles_returns'] = $this->model_openbay_ebay_profile->getAll(1);
				//get default returns profile
				$case_info['profiles_returns_def'] = $this->model_openbay_ebay_profile->getDefault(1);
				$this->data['data']['shipping_international_zones']     = $this->model_openbay_ebay->getShippingLocations();

				//get theme profiles
				$case_info['profiles_theme'] = $this->model_openbay_ebay_profile->getAll(2);
				//get default returns profile
				$case_info['profiles_theme_def'] = $this->model_openbay_ebay_profile->getDefault(2);

				//get generic profiles
				$case_info['profiles_generic'] = $this->model_openbay_ebay_profile->getAll(3);
				//get default generic profile
				$case_info['profiles_generic_def'] = $this->model_openbay_ebay_profile->getDefault(3);

				//case attributes - this is just a direct pass through used with the template tag
				$case_info['attributes'] = base64_encode(json_encode($this->model_openbay_ebay->getCaseAttributes($this->request->get['case_id'])));

				//post edit link
				$case_info['edit_link'] = $this->url->link('openbay/openbay/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'], 'SSL');

				//images
				$case_images = $this->model_catalog_case->getCaseImages($this->request->get['case_id']);
				$case_info['case_images'] = array();

				if (!empty($case_info['image'])) {
					$case_info['case_images'][] = array(
						'image' => $case_info['image'],
						'preview' => $this->model_tool_image->resize($case_info['image'], 100, 100),
						'full' => HTTPS_CATALOG . 'image/' . $case_info['image']
					);
				}
				...
				$case_info['manufacturers']                      = $this->model_catalog_manufacturer->getManufacturers();
				$case_info['payments']                           = $this->model_openbay_ebay->getPaymentTypes();
				$case_info['templates']                          = $this->model_openbay_ebay_template->getAll();
				$case_info['store_cats']                         = $this->model_openbay_ebay->getSellerStoreCategories();

				$case_info['defaults']['ebay_payment_types']     = $this->config->get('ebay_payment_types');
				$case_info['defaults']['paypal_address']         = $this->config->get('field_payment_paypal_address');
				$case_info['defaults']['payment_instruction']    = $this->config->get('field_payment_instruction');
				$case_info['defaults']['payment_immediate']      = $this->config->get('payment_immediate');

				$case_info['defaults']['gallery_height']         = '400';
				$case_info['defaults']['gallery_width']          = '400';
				$case_info['defaults']['thumb_height']           = '100';
				$case_info['defaults']['thumb_width']            = '100';


				$case_info['defaults']['listing_duration'] = $this->config->get('openbaypro_duration');
				if ($case_info['defaults']['listing_duration'] == '') {
					$case_info['defaults']['listing_duration'] = 'Days_30';
				}
				...
				if($case_info['quantity'] < 1 && (!isset($case_info['has_option']) || $case_info['has_option'] == 0)) {
		...
		if ($this->checkConfig() == true) {
			if(!empty($this->request->post['selected'])) {
				...
				foreach ($this->request->post['selected'] as $case_id) {
					if(!array_key_exists($case_id, $activeList)) {

						$prod = $this->model_catalog_case->getCase($case_id);
				...
				if($plan['plan']['listing_bulk'] == 1) {
					if($this->data['count'] == 0) {
						$this->data['error_fail'][] = $this->language->get('lang_error_no_case');
					}else{
						if(($plan['plan']['listing_limit'] == 0) || (($plan['usage']['items'] + $this->data['count']) <= $plan['plan']['listing_limit'])) {
							...
							//get default generic profile
							$case_info['profiles_generic_def'] = $this->model_openbay_ebay_profile->getDefault(3);
							if($case_info['profiles_generic_def'] === false) {
							...
							if($case_info['profiles_shipping_def'] === false) {
							...
							$case_info['profiles_returns_def'] = $this->model_openbay_ebay_profile->getDefault(1);
							//check it has a default profile
							if($case_info['profiles_returns_def'] === false) {
							...
							$case_info['defaults']['ebay_template'] = $this->config->get('ebay_template');
	...
	public function verify() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ($this->checkConfig() == true) {
				...
				$item_id = $this->openbay->ebay->getEbayItemId($this->request->post['case_id']);
	...
	public function verifyBulk() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ($this->checkConfig() == true) {
				...
				$case_info               = $this->model_catalog_case->getCase($post['case_id']);

				$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, p.sort_order FROM " . DB_PREFIX . "case p LEFT JOIN " . DB_PREFIX . "case_description pd ON (p.case_id = pd.case_id) LEFT JOIN " . DB_PREFIX . "case_to_store p2s ON (p.case_id = p2s.case_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.case_id = '" . (int)$post['case_id'] . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
				...
				$data['case_id']         = (int)$post['case_id'];

				$data['feat']           	= $post['feat'];
				$data['featother']          = $post['featother'];

				if(!empty($case_info['sku'])){
					$data['sku'] = $case_info['sku'];
				}
				...
				//case attributes - this is just a direct pass through used with the template tag
				$data['attributes'] = base64_encode(json_encode($this->model_openbay_ebay->getCaseAttributes($post['case_id'])));
				...
				$case_images = $this->model_catalog_case->getCaseImages($post['case_id']);

				$case_info['case_images'] = array();

				if (!empty($case_info['image'])) {
					$data['img'][] = $case_info['image'];
				}

				if(isset($profile_template['data']['ebay_img_ebay']) && $profile_template['data']['ebay_img_ebay'] == 1) {
					foreach ($case_images as $case_image) {
						if ($case_image['image'] && file_exists(DIR_IMAGE . $case_image['image'])) {
							$data['img'][] =  $case_image['image'];
						}
					}
				}
				...
				if(isset($profile_template['data']['ebay_img_template']) && $profile_template['data']['ebay_img_template'] == 1) {
					...
					if(!isset($profile_template['data']['default_img_exclude']) || $profile_template['data']['default_img_exclude'] != 1) {
						$tmpGalArray[0] = $this->model_tool_image->resize($case_info['image'], $profile_template['data']['ebay_gallery_width'], $profile_template['data']['ebay_gallery_height']);
						$tmpThumbArray[0] = $this->model_tool_image->resize($case_info['image'], $profile_template['data']['ebay_thumb_width'], $profile_template['data']['ebay_thumb_height']);
						$keyOffset = 1;
					}

					//loop through the case images and add them.
	...
	public function repairLinks() {
		...
		$this->model_openbay_ebay_case->repairLinks();
