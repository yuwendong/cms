...
class ControllerOpenbayAmazon extends Controller {
	...
	public function overview() {
		...
		$this->data['link_bulk_listing'] = $this->url->link('openbay/amazon/bulkListCases', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function settings() {
		...
		$this->data['itemLinks_url'] = $this->url->link('openbay/amazon_case/linkItems', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function savedListings() {
		...
		$this->data['token'] = $this->session->data['token'];
		$this->load->model('openbay/amazon');
		$saved_cases = $this->model_openbay_amazon->getSavedCases();
		...
		foreach($saved_cases as $saved_case) {
			$this->data['saved_cases'][] = array(
				...
				'amazon_sku' => $saved_case['amazon_sku'],
				'var' => $saved_case['var'],
				'edit_link' => $this->url->link('openbay/amazon_case', 'token=' . $this->session->data['token'] . '&case_id=' . $saved_case['case_id'] . '&var=' . $saved_case['var'], 'SSL'),
			);
		}

		$this->data['deleteSavedAjax'] = $this->url->link('openbay/amazon/deleteSavedAjax', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['uploadSavedAjax'] = $this->url->link('openbay/amazon_case/uploadSavedAjax', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function getOpenstockOptionsAjax() {
		...
		if($this->openbay->addonLoad('openstock') && isset($this->request->get['case_id'])) {
			...
			$json = $this->model_openstock_openstock->getCaseOptionStocks($this->request->get['case_id']);
		}
		if(empty($json)) {
			$json = false;
		}
	...
	public function addItemLinkAjax() {
		...
		if(isset($this->request->get['case_id']) && isset($this->request->get['amazon_sku'])) {
			$this->load->model('openbay/amazon');
			...
			$this->model_openbay_amazon->linkCase($amazon_sku, $case_id, $var);
			$logger = new Log('amazon_stocks.log');
			$logger->write('addItemLink() called for case id: ' . $case_id . ', amazon sku: ' . $amazon_sku . ', var: ' . $var);
			...
			if($var != '' && $this->openbay->addonLoad('openstock')) {
				...
				$this->openbay->amazon->putStockUpdateBulk(array($case_id));
			}
	...
	public function removeItemLinkAjax() {
		...
		if(isset($this->request->get['amazon_sku'])) {
			...
			$this->model_openbay_amazon->removeCaseLink($amazon_sku);
	...
	public function getItemLinksAjax() {
		...
		$json = json_encode($this->model_openbay_amazon->getCaseLinks());
	...
	public function getUnlinkedItemsAjax() {
		...
		$json = json_encode($this->model_openbay_amazon->getUnlinkedCases());
	...
	public function deleteSavedAjax() {
		...
		$this->load->model('openbay/amazon');
		$this->model_openbay_amazon->deleteSaved($this->request->get['case_id'], $this->request->get['var']);
	}
	...
	public function bulkListCases() {
		...
		$this->data['breadcrumbs'][] = array(
			'href'      => $this->url->link('openbay/amazon/bulkListCases', 'token=' . $this->session->data['token'], 'SSL'),
		...
		if ($bulk_listing_status) {
			...
			$results = $this->model_openbay_amazon->getCaseSearch($data);
			$case_total = $this->model_openbay_amazon->getCaseSearchTotal($data);
			...
			$pagination->url = $this->url->link('openbay/amazon/bulkListCases', 'token=' . $this->session->data['token'] . '&page={page}&filter_marketplace=' . $filter_marketplace, 'SSL');
	...
	public function doBulkLinking() {
		...
		if (!empty($links)) {
			foreach ($links as $link) {
				$this->model_openbay_amazon->linkCase($link['amazon_sku'], $link['case_id'], $link['var']);
			}
