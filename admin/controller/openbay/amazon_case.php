<?php
class ControllerOpenbayAmazonCase extends Controller {
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_openbay_amazon->saveCase($case_id, $dataArray);
			...
			if($dataArray['upload_after'] === 'true') {
				...
				$this->redirect($this->url->link('openbay/amazon_case', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'));
			}
		}
		...
		$savedListingData = $this->model_openbay_amazon->getCase($case_id, $variation);
		...
		$errors = $this->model_openbay_amazon->getCaseErrors($case_id);
		...
		if($listingSaved) {
			$this->data['template_parser_url'] = $this->url->link('openbay/amazon_case/parseTemplateAjax&edit_id=' . $case_id, 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$this->data['template_parser_url'] = $this->url->link('openbay/amazon_case/parseTemplateAjax&case_id=' . $case_id, 'token=' . $this->session->data['token'], 'SSL');
		}

		$this->data['url_remove_errors'] = $this->url->link('openbay/amazon_case/removeErrors', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL');
		...
		$this->data['main_url'] = $this->url->link('openbay/amazon_case', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	public function removeErrors() {
		...
		$this->load->model('openbay/amazon');
		$this->model_openbay_amazon->removeAdvancedErrors($case_id);
	...
	private function uploadSaved() {
		...
		$savedCases = $this->model_openbay_amazon->getSavedCasesData();
		...
		foreach($savedCases as $savedCase) {
			...
			$response_data = array("response_url" => $catalog . 'index.php?route=amazon/case/inbound');
			$category_data = array('category' => (string)$savedCase['category']);
			$fields_data = array('fields' => (array)$caseDataDecoded['fields']);

			$mpArray = !empty($savedCase['marketplaces']) ? (array)unserialize($savedCase['marketplaces']) : array();
			...
			$insertion_response = $this->openbay->amazon->insertCase($caseData);
			...
			$this->model_openbay_amazon->setCaseUploaded($savedCase['case_id'], $insertion_response['insertion_id'], $savedCase['var']);
		}
	...
	public function parseTemplateAjax() {
		...
		$log = new Log('amazon_case.log');

		$json = array();

		if(isset($this->request->get['xml'])) {
			$request = array('template' => $this->request->get['xml'], 'version' => 2);
			$response = $this->openbay->amazon->callWithResponse("casev2/GetTemplateXml", $request);
			if ($response) {
				...
				$log->write("admin/openbay/amazon_case/parseTemplateAjax failed calling casev2/GetTemplateXml with params: " . print_r($request, true));
			}
		}
	...
	private function fillSavedValues($case_id, $fields_array, $var = '') {
		...
		$this->load->model('openbay/amazon');
		$savedListing = $this->model_openbay_amazon->getCase($case_id, $var);
		...
		$saved_fields['Quantity'] = $this->model_openbay_amazon->getCaseQuantity($case_id, $var);
	...
	public function resetPending() {
		$this->db->query("UPDATE `" . DB_PREFIX . "amazon_case` SET `status` = 'saved' WHERE `status` = 'uploaded'");
	}
