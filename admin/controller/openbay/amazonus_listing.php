...
class ControllerOpenbayAmazonusListing extends Controller{
	public function create() {
		...
		if ($this->request->post) {
			$case = $this->request->post;
			if (isset($case['option_variant'])) {
				$variant = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case_option_relation` WHERE `var` = '" . $this->db->escape((string)$case['option_variant']) . "' AND `case_id` = '".(int)$case['case_id']."' LIMIT 1")->row;
				$case['var'] = $case['option_variant'];
				$case['quantity'] = $variant['stock'];
			} else {
				$case_info = $this->model_catalog_case->getCase($case['case_id']);
				$case['var'] = '';
				$case['quantity'] = $case_info['quantity'];
			}

			$result = $this->model_openbay_amazonus_listing->simpleListing($case);

			if($result['status'] === 1) {
				$this->session->data['success'] = $this->language->get('text_case_sent');
				$this->redirect($this->url->link('extension/openbay/itemList', 'token=' . $this->session->data['token'] . $url, 'SSL'));
			} else {
				$this->session->data['error'] = sprintf($this->language->get('text_case_not_sent'), $result['message']);
				$this->redirect($this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->post['case_id'] . $url, 'SSL'));
			}
		}
		...
		if (isset($this->request->get['case_id'])) {
			...
			if($this->openbay->addonLoad('openstock') == true) {
				...
				$this->data['options'] = $this->model_openstock_openstock->getCaseOptionStocks($this->request->get['case_id']);
			} else {
				$this->data['options'] = array();
			}

			$listing_status = $this->model_openbay_amazonus->getCaseStatus($this->request->get['case_id']);

			if($listing_status === 'processing' || $listing_status === 'ok') {
				$this->redirect($this->url->link('openbay/amazonus_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'] . $url, 'SSL'));
			} else if ($listing_status === 'error_advanced' || $listing_status === 'saved' || $listing_status === 'error_few') {
				$this->redirect($this->url->link('openbay/amazonus_case', 'token=' . $this->session->data['token'] . '&case_id=' . $this->request->get['case_id'] . $url, 'SSL'));
			}
		} else {
			$this->redirect($this->url->link('extension/openbay/itemList', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
		...
		$this->data['url_advanced']  = $this->url->link('openbay/amazonus_case', 'token=' . $this->session->data['token'] . '&case_id='.$this->request->get['case_id'] . $url, 'SSL');
		...
		$this->data['sku'] = trim($case_info['sku']);

		if ($this->config->get('openbay_amazonus_listing_tax_added')) {
				$this->data['price'] = $case_info['price'] * (1 + $this->config->get('openbay_amazonus_listing_tax_added') / 100);
			} else {
		...
		if ($listing_status == 'error_quick') {
			$this->data['listing_errors'] = $this->model_openbay_amazonus->getCaseErrors($case_info['case_id'], 3);
		}

		$this->data['price'] = number_format($this->data['price'], 2);
		$this->data['quantity'] = $case_info['quantity'];
		$this->data['case_id'] = $case_info['case_id'];
	...
	public function edit() {
		...
		$status = $this->model_openbay_amazonus->getCaseStatus($case_id);

		//If case was not submited/saved for Amazonus
		if($status === false) {
			$this->redirect($this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'));
			return;
		}

		$this->data['case_links'] = $this->model_openbay_amazonus->getCaseLinks($case_id);
		$this->data['url_return']  = $this->url->link('extension/openbay/itemList', 'token=' . $this->session->data['token'] . $url, 'SSL');
		if($status == 'ok' || $status == 'linked') {
			$this->data['url_create_new']  = $this->url->link('openbay/amazonus_listing/createNew', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL');
			$this->data['url_delete_links']  = $this->url->link('openbay/amazonus_listing/deleteLinks', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL');
		}
		...
		$this->data['url_saved_listings']  = $this->url->link('openbay/amazonus/savedListings', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id, 'SSL');
		...
		$this->data['breadcrumbs'][] = array(
			'href' => $this->url->link('openbay/amazonus_listing/edit', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'),
			'text' => $this->language->get('text_edit_heading'),
			'separator' => ' :: '
		);

		$this->data['text_edit_heading'] = $this->language->get('text_edit_heading');
		$this->data['text_case_links'] = $this->language->get('text_case_links');
	...
	public function createNew() {
		...
		$this->load->model('openbay/amazonus');
		$this->model_openbay_amazonus->deleteCase($case_id);
		$this->redirect($this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&case_id=' . $case_id . $url, 'SSL'));
	}
	...
	public function deleteLinks() {
		...
		$links = $this->model_openbay_amazonus->getCaseLinks($case_id);
		foreach ($links as $link) {
			$this->model_openbay_amazonus->removeCaseLink($link['amazonus_sku']);
		}
		$this->model_openbay_amazonus->deleteCase($case_id);
	...
	public function getCaseByAsin() {


		$this->load->model('openbay/amazonus_listing');

		$data = $this->model_openbay_amazonus_listing->getCaseByAsin($this->request->post['asin']);
