...
class ControllerCommonHome extends Controller {   
	public function index() {
		...
		$this->data['text_total_case'] = $this->language->get('text_total_case');
		...
		$this->data['column_case_model'] = $this->language->get('column_case_model');
		$this->data['column_case_name'] = $this->language->get('column_case_name');
		$this->data['column_case_tag'] = $this->language->get('column_case_tag');		
		$this->data['column_case_figure'] = $this->language->get('column_case_figure');		
		$this->data['column_case_spread'] = $this->language->get('column_case_spread');		
		$this->data['column_case_time'] = $this->language->get('column_case_time');		
		$this->data['column_case_location'] = $this->language->get('column_case_location');
		...
		$this->data['total_case'] = $this->model_catalog_case->getTotalCases();
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/case/update', 'token=' . $this->session->data['token'] . '&case_id=' . $result['case_id'], 'SSL')
			);
			...
			$this->data['cases'][] = array(
				...
				'date_unavailable' => $result['date_unavailable'],
				'action'     => $action
			);
		}
	...
	public function chart() {
		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "case` WHERE (DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "') GROUP BY HOUR(date_added) ORDER BY date_added ASC");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND YEAR(date_added) = '2014' AND MONTH(date_added) = '" . $i . "' GROUP BY MONTH(date_added)");
