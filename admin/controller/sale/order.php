...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		$this->data['entry_case'] = $this->language->get('entry_case');
		...
		$this->data['column_case'] = $this->language->get('column_case');
		...
		if (isset($this->request->post['order_case'])) {
			$order_cases = $this->request->post['order_case'];
		} elseif (isset($this->request->get['order_id'])) {
			$order_cases = $this->model_sale_order->getOrderCases($this->request->get['order_id']);
		} else {
			$order_cases = array();
		}
		...
		$this->data['order_cases'] = array();

		foreach ($order_cases as $order_case) {
			if (isset($order_case['order_option'])) {
				$order_option = $order_case['order_option'];
			} elseif (isset($this->request->get['order_id'])) {
				$order_option = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $order_case['order_case_id']);
			} else {
				$order_option = array();
			}

			if (isset($order_case['order_download'])) {
				$order_download = $order_case['order_download'];
			} elseif (isset($this->request->get['order_id'])) {
				$order_download = $this->model_sale_order->getOrderDownloads($this->request->get['order_id'], $order_case['order_case_id']);
			} else {
				$order_download = array();
			}

			$this->data['order_cases'][] = array(
				'order_case_id' => $order_case['order_case_id'],
				'case_id'       => $order_case['case_id'],
				'name'             => $order_case['name'],
				'model'            => $order_case['model'],
				'option'           => $order_option,
				'download'         => $order_download,
				'quantity'         => $order_case['quantity'],
				'price'            => $order_case['price'],
				'total'            => $order_case['total'],
				'tax'              => $order_case['tax'],
				'reward'           => $order_case['reward']
			);
		}
	...
	protected function validateForm() {
		...
		// Check if any cases require shipping
		$shipping = false;

		if (isset($this->request->post['order_case'])) {
			$this->load->model('catalog/case');

			foreach ($this->request->post['order_case'] as $order_case) {
				$case_info = $this->model_catalog_case->getCase($order_case['case_id']);

				if ($case_info && $case_info['shipping']) {
					$shipping = true;
				}
			}
		}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$cases = $this->model_sale_order->getOrderCases($this->request->get['order_id']);

			foreach ($cases as $case) {
				$option_data = array();

				$options = $this->model_sale_order->getOrderOptions($this->request->get['order_id'], $case['order_case_id']);
				...
				$this->data['cases'][] = array(
					'order_case_id' => $case['order_case_id'],
			...
			foreach ($cases as $case) {
				$results = $this->model_sale_order->getOrderDownloads($this->request->get['order_id'], $case['order_case_id']);
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$cases = $this->model_sale_order->getOrderCases($order_id);

				foreach ($cases as $case) {
					$option_data = array();

					$options = $this->model_sale_order->getOrderOptions($order_id, $case['order_case_id']);
