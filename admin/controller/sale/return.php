...
class ControllerSaleReturn extends Controller { 
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if (isset($this->request->get['filter_case'])) {
				$url .= '&filter_case=' . urlencode(html_entity_decode($this->request->get['filter_case'], ENT_QUOTES, 'UTF-8'));
			}
	...
	protected function getList() {
		...
		$this->data['sort_case'] = $this->url->link('sale/return', 'token=' . $this->session->data['token'] . '&sort=case' . $url, 'SSL');
		...
		$this->data['filter_case'] = $filter_case;
	...
	protected function getForm() {
		...
		if (isset($this->request->post['case'])) {
			...
			$this->data['case'] = $return_info['case'];
		} else {
			$this->data['case'] = '';
		}	

		if (isset($this->request->post['case_id'])) {
			$this->data['case_id'] = $this->request->post['case_id'];
		} elseif (!empty($return_info)) {
			$this->data['case_id'] = $return_info['case_id'];
		} else {
			$this->data['case_id'] = '';
		}	
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['case']) < 1) || (utf8_strlen($this->request->post['case']) > 255)) {
