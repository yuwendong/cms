...
class ModelSaleOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['order_case'])) {
			foreach ($data['order_case'] as $order_case) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_case SET order_id = '" . (int)$order_id . "', case_id = '" . (int)$order_case['case_id'] . "', name = '" . $this->db->escape($order_case['name']) . "', model = '" . $this->db->escape($order_case['model']) . "', quantity = '" . (int)$order_case['quantity'] . "', price = '" . (float)$order_case['price'] . "', total = '" . (float)$order_case['total'] . "', tax = '" . (float)$order_case['tax'] . "', reward = '" . (int)$order_case['reward'] . "'");

				$order_case_id = $this->db->getLastId();

				$this->db->query("UPDATE " . DB_PREFIX . "`case` SET quantity = (quantity - " . (int)$order_case['quantity'] . ") WHERE case_id = '" . (int)$order_case['case_id'] . "' AND subtract = '1'");

				if (isset($order_case['order_option'])) {
					foreach ($order_case['order_option'] as $order_option) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_case_id = '" . (int)$order_case_id . "', case_option_id = '" . (int)$order_option['case_option_id'] . "', case_option_value_id = '" . (int)$order_option['case_option_value_id'] . "', name = '" . $this->db->escape($order_option['name']) . "', `value` = '" . $this->db->escape($order_option['value']) . "', `type` = '" . $this->db->escape($order_option['type']) . "'");

						$this->db->query("UPDATE " . DB_PREFIX . "case_option_value SET quantity = (quantity - " . (int)$order_case['quantity'] . ") WHERE case_option_value_id = '" . (int)$order_option['case_option_value_id'] . "' AND subtract = '1'");
					}
				}

				if (isset($order_case['order_download'])) {
					foreach ($order_case['order_download'] as $order_download) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_download SET order_id = '" . (int)$order_id . "', order_case_id = '" . (int)$order_case_id . "', name = '" . $this->db->escape($order_download['name']) . "', filename = '" . $this->db->escape($order_download['filename']) . "', mask = '" . $this->db->escape($order_download['mask']) . "', remaining = '" . (int)$order_download['remaining'] . "'");
					}
				}
			}
		}
	...
	public function editOrder($order_id, $data) {
		...
		// Restock cases before subtracting the stock later on
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_status_id > '0' AND order_id = '" . (int)$order_id . "'");

		if ($order_query->num_rows) {
			$case_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_case WHERE order_id = '" . (int)$order_id . "'");

			foreach($case_query->rows as $case) {
				$this->db->query("UPDATE `" . DB_PREFIX . "case` SET quantity = (quantity + " . (int)$case['quantity'] . ") WHERE case_id = '" . (int)$case['case_id'] . "' AND subtract = '1'");

				$option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_case_id = '" . (int)$case['order_case_id'] . "'");

				foreach ($option_query->rows as $option) {
					$this->db->query("UPDATE " . DB_PREFIX . "case_option_value SET quantity = (quantity + " . (int)$case['quantity'] . ") WHERE case_option_value_id = '" . (int)$option['case_option_value_id'] . "' AND subtract = '1'");
				}
			}
		}
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_case WHERE order_id = '" . (int)$order_id . "'"); 
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_download WHERE order_id = '" . (int)$order_id . "'");

		if (isset($data['order_case'])) {
			foreach ($data['order_case'] as $order_case) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_case SET order_case_id = '" . (int)$order_case['order_case_id'] . "', order_id = '" . (int)$order_id . "', case_id = '" . (int)$order_case['case_id'] . "', name = '" . $this->db->escape($order_case['name']) . "', model = '" . $this->db->escape($order_case['model']) . "', quantity = '" . (int)$order_case['quantity'] . "', price = '" . (float)$order_case['price'] . "', total = '" . (float)$order_case['total'] . "', tax = '" . (float)$order_case['tax'] . "', reward = '" . (int)$order_case['reward'] . "'");
				...
				if (isset($order_case['order_option'])) {
					foreach ($order_case['order_option'] as $order_option) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_option_id = '" . (int)$order_option['order_option_id'] . "', order_id = '" . (int)$order_id . "', order_case_id = '" . (int)$order_case_id . "', case_option_id = '" . (int)$order_option['case_option_id'] . "', case_option_value_id = '" . (int)$order_option['case_option_value_id'] . "', name = '" . $this->db->escape($order_option['name']) . "', `value` = '" . $this->db->escape($order_option['value']) . "', `type` = '" . $this->db->escape($order_option['type']) . "'");
				...
				if (isset($order_case['order_download'])) {
					foreach ($order_case['order_download'] as $order_download) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "order_download SET order_download_id = '" . (int)$order_download['order_download_id'] . "', order_id = '" . (int)$order_id . "', order_case_id = '" . (int)$order_case_id . "', name = '" . $this->db->escape($order_download['name']) . "', filename = '" . $this->db->escape($order_download['filename']) . "', mask = '" . $this->db->escape($order_download['mask']) . "', remaining = '" . (int)$order_download['remaining'] . "'");
					}
				}
			}
		}
	...
	public function getOrderCases($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_case WHERE order_id = '" . (int)$order_id . "'");
	...
	public function getOrderOptions($order_id, $order_case_id) {
		$query = $this->db->query("SELECT oo.* FROM " . DB_PREFIX . "order_option AS oo LEFT JOIN " . DB_PREFIX . "case_option po USING(case_option_id) LEFT JOIN `" . DB_PREFIX . "option` o USING(option_id) WHERE order_id = '" . (int)$order_id . "' AND order_case_id = '" . (int)$order_case_id . "' ORDER BY o.sort_order");

		return $query->rows;
	}

	public function getOrderDownloads($order_id, $order_case_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_download WHERE order_id = '" . (int)$order_id . "' AND order_case_id = '" . (int)$order_case_id . "'");
	...
	public function getEmailsByCasesOrdered($cases, $start, $end) {
		$implode = array();

		foreach ($cases as $case_id) {
			$implode[] = "op.case_id = '" . (int)$case_id . "'";
		}

		$query = $this->db->query("SELECT DISTINCT email FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_case op ON (o.order_id = op.order_id) WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0' LIMIT " . (int)$start . "," . (int)$end);

		return $query->rows;
	}

	public function getTotalEmailsByCasesOrdered($cases) {
		$implode = array();
		...
		$query = $this->db->query("SELECT DISTINCT email FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_case op ON (o.order_id = op.order_id) WHERE (" . implode(" OR ", $implode) . ") AND o.order_status_id <> '0'");
