...
class ModelPaymentAmazonCheckout extends Model {
	public function install() {
		...
		$this->db->query("
			CREATE TABLE `" . DB_PREFIX . "order_amazon_case` (
			`order_case_id`  int NOT NULL ,
			`amazon_order_item_code`  varchar(255) NOT NULL,
			PRIMARY KEY (`order_case_id`)
		) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
		");
	...
	public function uninstall() {
		...
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "order_amazon_case`;");
	...
	public function getOrder($order_id) {
		...
		if ($result->num_rows) {
			...
			$results = $this->db->query("SELECT oap.order_case_id, amazon_order_item_code, op.quantity FROM " . DB_PREFIX . "order_amazon_case oap JOIN " . DB_PREFIX . "order_case op USING(order_case_id) WHERE order_id = " . (int)$order_id . "
			")->rows;

			foreach ($results as $result) {
				$order['cases'][$result['order_case_id']] = array(
