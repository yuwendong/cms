...
class ModelOpenbayOpenbay extends Model {
	...
	public function getTotalCases($data = array()) {
		...
		if ($data['filter_market_name'] == 'ebay') {
			$sql .= " LEFT JOIN `" . DB_PREFIX . "ebay_listing` `ebay` ON (`p`.`case_id` = `ebay`.`case_id`)";

			if ($data['filter_market_id'] == 0) {
				$sql .= " LEFT JOIN (SELECT case_id, IF( SUM( `status` ) = 0, 0, 1 ) AS 'listing_status' FROM " . DB_PREFIX . "ebay_listing GROUP BY case_id ) ebay2 ON (p.case_id = ebay2.case_id)";
			}
		}

		if ($data['filter_market_name'] == 'amazon') {
			if ($data['filter_market_id'] <= 4) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "amazon_case ap ON p.case_id = ap.case_id";
			} else {
				$sql .= " LEFT JOIN " . DB_PREFIX . "amazon_case_link apl ON p.case_id = apl.case_id";
			}
	...
	public function getCases($data = array()) {
		$sql = "SELECT p.*, pd.* FROM " . DB_PREFIX . "`case` p LEFT JOIN " . DB_PREFIX . "case_description pd ON (p.case_id = pd.case_id)";
		...
		if ($data['filter_market_name'] == 'amazonus') {
			if ($data['filter_market_id'] <= 4) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "amazonus_case ap ON p.case_id = ap.case_id";
			} elseif ($data['filter_market_id'] <= 6) {
				$sql .= " LEFT JOIN " . DB_PREFIX . "amazonus_case_link apl ON p.case_id = apl.case_id";
			}
