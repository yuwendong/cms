...
;
	...
	public function totalLinked(){
		$sql = "SELECT COUNT(DISTINCT p.case_id) AS total
				...
				LEFT JOIN `" . DB_PREFIX . "case` `p` ON (`el`.`case_id` = `p`.`case_id`)
				LEFT JOIN `" . DB_PREFIX . "case_description` `pd` ON (`p`.`case_id` = `pd`.`case_id`)
				WHERE `el`.`status` = '1'
	...
	public function loadLinked($limit = 100, $page = 1){
		...
		LEFT JOIN `" . DB_PREFIX . "case` `p` ON (`el`.`case_id` = `p`.`case_id`)
		LEFT JOIN `" . DB_PREFIX . "case_description` `pd` ON (`p`.`case_id` = `pd`.`case_id`)
		LEFT JOIN `" . DB_PREFIX . "ebay_stock_reserve` `esr` ON (`esr`.`case_id` = `p`.`case_id`)
		WHERE `el`.`status` = '1'
		AND `pd`.`language_id` = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if($qry->num_rows){
			foreach($qry->rows as $row){
				$data[$row['ebay_item_id']] = array(
					...
					'qty'           => $row['quantity'],
					'name'          => $row['name'],
					'link_edit'     => $this->url->link('catalog/case/update', 'token=' . $this->session->data['token'] . '&case_id='.$row['case_id'], 'SSL'),
				...
				if ((isset($row['has_option']) && $row['has_option'] == 1) && $this->openbay->addonLoad('openstock')) {
					$data[$row['ebay_item_id']]['options'] = $this->model_openstock_openstock->getCaseOptionStocks((int)$row['case_id']);
				}
				...
				if($this->config->get('openbaypro_stock_allocate') == 0){
					$data[$row['ebay_item_id']]['allocated'] = $this->openbay->ebay->getAllocatedStock($row['case_id']);
				}else{
	...
	public function loadUnlinked($limit = 200, $page = 1, $filter = array()){
		...
		while(count($unlinked) < 5){
			...
			foreach($response['items'] as $itemId => $item){
				if($this->openbay->ebay->getCaseId($itemId, 1) == false){
	...
	public function saveItemLink($data) {
		...
		if(($data['qty'] != $data['ebayqty']) || $data['variants'] == 1){
			...
			$this->openbay->ebay->caseUpdateListen($data['pid'], $this->model_catalog_case->getCase($data['pid']));
		}else{
	...
	public function ebayAddItem($data, $options){
		...
		if(!empty($response['ItemID'])){
			$this->openbay->ebay->createLink($data['case_id'], $response['ItemID'], $variant);
	...
	public function getCaseStock($id){
		$res = $this->db->query("SELECT * FROM `" . DB_PREFIX . "case` WHERE `case_id` = '".$this->db->escape($id)."' LIMIT 1");

		if (isset($res->row['has_option']) && $res->row['has_option'] == 1) {
			if ($this->openbay->addonLoad('openstock')) {
				$this->load->model('openstock/openstock');
				$this->load->model('tool/image');
				$variant        = $this->model_openstock_openstock->getCaseOptionStocks((int)$id);
			}else{
				$variant        = 0;
			}
		}else{
	...
	public function editSave($data){
		...
		//get case id
		$case_id = $this->openbay->ebay->getCaseId($data['itemId']);

		$this->openbay->ebay->log('editSave() - case_id: '.$case_id);

		if($data['variant'] == 0){
			//save the reserve level
			$this->openbay->ebay->updateReserve($case_id, $data['itemId'], $data['qty_reserve']);
			...
			//get the options list for this case
			$opts = $this->model_openstock_openstock->getCaseOptionStocks($case_id);
			reset($opts);
			...
			foreach($data['opt'] as $k => $opt){
				...
				$this->openbay->ebay->updateReserve($case_id, $data['itemId'], $opt['reserve'], $opt['sku'], 1);

				//get the stock info
				$stock = $this->openbay->ebay->getCaseStockLevel($case_id, $opt['sku']);
				...
				$reserve = $this->openbay->ebay->getReserve($case_id, $data['itemId'], $opt['sku']);
