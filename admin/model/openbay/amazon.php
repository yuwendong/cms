...
class ModelOpenbayAmazon extends Model {
  ...
  <MarketplaceIDs>';
	...
	public function deleteSaved($case_id, $var = '') {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "amazon_case`
			WHERE `case_id` = '" . (int)$case_id . "' AND `var` = '" . $this->db->escape($var) . "'");
	}

	public function getSavedCases() {
		return $this->db->query("
			SELECT `ap`.`status`, `ap`.`case_id`, `ap`.`sku` as `amazon_sku`, `pd`.`name` as `case_name`, `p`.`model` as `case_model`, `p`.`sku` as `case_sku`, `ap`.`var` as `var`
			FROM `" . DB_PREFIX . "amazon_case` as `ap`
	...
	public function getCaseCategory($case_id, $var = '') {
		$row = $this->db->query("
			SELECT `category` FROM `" . DB_PREFIX . "amazon_case`
			WHERE `case_id` = '" . (int)$case_id . "' AND `var` = '" . $this->db->escape($var) . "' AND `version` = 2")->row;
	...
	public function getCaseStatus($case_id) {
		...
		$links = $this->db->query("
			...
			FROM `" . DB_PREFIX . "amazon_case_link`
	...
	public function getCaseErrors($case_id, $version = 2) {
		if($version == 3) {
			...
			SELECT `messages` FROM `" . DB_PREFIX . "amazon_case`
			WHERE `case_id` = '" . (int)$case_id . "' AND `version` = 3")->row;
		...
		$insertionRows = $this->db->query("
			SELECT `sku`, `insertion_id` FROM `" . DB_PREFIX . "amazon_case`
		...
		if(!empty($insertionRows)) {
			foreach($insertionRows as $insertionRow) {
				$errorRows = $this->db->query("
					SELECT * FROM `" . DB_PREFIX . "amazon_case_error`
	...
	public function getCasesWithErrors() {
		return $this->db->query("
			SELECT `case_id`, `sku` FROM `" . DB_PREFIX . "amazon_case`
			WHERE `status` = 'error' AND `version` = 2")->rows;
	}

	public function deleteCase($case_id) {
		$this->db->query(
			"DELETE FROM `" . DB_PREFIX . "amazon_case`
			WHERE `case_id` = '" . (int)$case_id . "'");
	}

	public function linkCase($amazon_sku, $case_id, $var = '') {
		$count = $this->db->query("SELECT COUNT(*) as 'count' FROM `" . DB_PREFIX . "amazon_case_link` WHERE `case_id` = '" . (int)$case_id . "' AND `amazon_sku` = '" . $this->db->escape($amazon_sku) . "' AND `var` = '" . $this->db->escape($var) . "' LIMIT 1")->row;
		if($count['count'] == 0) {
			$this->db->query(
				"INSERT INTO `" . DB_PREFIX . "amazon_case_link`
				SET `case_id` = '" . (int)$case_id . "', `amazon_sku` = '" . $this->db->escape($amazon_sku) . "', `var` = '" . $this->db->escape($var) . "'");
		}
	}

	public function removeCaseLink($amazon_sku) {
		$this->db->query(
			"DELETE FROM `" . DB_PREFIX . "amazon_case_link`
			WHERE `amazon_sku` = '" . $this->db->escape($amazon_sku) . "'");
	}

	public function removeAdvancedErrors($case_id) {
		$case_rows = $this->db->query("
			SELECT `insertion_id` FROM `" . DB_PREFIX . "amazon_case`
			WHERE `case_id` = '" . (int)$case_id . "' AND `version` = 2")->rows;

		foreach ($case_rows as $case) {
			$this->db->query(
				"DELETE FROM `" . DB_PREFIX . "amazon_case_error`
	...
	public function getCaseLinks($case_id = 'all') {
		$query = "SELECT `apl`.`amazon_sku`, `apl`.`case_id`, `pd`.`name` as `case_name`, `p`.`model`, `p`.`sku`, `apl`.`var`, '' as `combi`
			FROM `" . DB_PREFIX . "amazon_case_link` as `apl`
			LEFT JOIN `" . DB_PREFIX . "case_description` as `pd`
	...
	public function getUnlinkedCases() {
		...
		if ($this->openbay->addonLoad('openstock')) {
			...
			$result = $this->db->query("
				...
				LEFT JOIN `" . DB_PREFIX . "case_description` as `pd`
				ON `p`.`case_id` = `pd`.`case_id`
				LEFT JOIN `" . DB_PREFIX . "amazon_case_link` as `apl`
				ON `apl`.`case_id` = `p`.`case_id`
	...
	private function caseLinkExists($case_id, $var) {
		$link = $this->db->query("SELECT * FROM `" . DB_PREFIX . "amazon_case_link` WHERE `case_id` = " . (int)$case_id . " AND var = '" . $this->db->escape($var) . "'")->row;
	...
	public function getAmazonOrderedCases($orderId) {
		return $this->db->query("
			SELECT `aop`.`amazon_order_item_id`, `op`.`quantity`
			FROM `" . DB_PREFIX . "amazon_order_case` `aop`
	...
	public function getCaseSearchTotal($data = array()) {
		$sql = "
			...
			LEFT JOIN " . DB_PREFIX . "amazon_case_search aps ON p.case_id = aps.case_id AND aps.marketplace = '" . $this->db->escape($data['filter_marketplace']) . "'
			LEFT JOIN " . DB_PREFIX . "amazon_case_link apl ON p.case_id = apl.case_id
			LEFT JOIN " . DB_PREFIX . "amazon_case ap ON p.case_id = ap.case_id
	...
	public function getCaseSearch($data = array()) {
		$sql = "
			SELECT p.case_id, marketplace, aps.status, aps.data, aps.matches
			FROM " . DB_PREFIX . "`case` p
			LEFT JOIN " . DB_PREFIX . "amazon_case_search aps ON p.case_id = aps.case_id AND aps.marketplace = '" . $this->db->escape($data['filter_marketplace']) . "'
			LEFT JOIN " . DB_PREFIX . "amazon_case_link apl ON p.case_id = apl.case_id
			LEFT JOIN " . DB_PREFIX . "amazon_case ap ON p.case_id = ap.case_id
	...
	public function updateAmazonSkusQuantities($skus) {
		...
		if ($this->openbay->addonLoad('openstock')) {
			$rows = $this->db->query("
				...
				FROM " . DB_PREFIX . "amazon_case_link apl
	...
	public function getTotalUnlinkedItemsFromReport($marketplace) {
		if ($this->openbay->addonLoad('openstock')) {
			$result = $this->db->query("
				...
				LEFT JOIN " . DB_PREFIX . "amazon_case_link apl ON (oc_sku.var IS NULL AND oc_sku.case_id = apl.case_id) OR (oc_sku.var IS NOT NULL AND oc_sku.case_id = apl.case_id AND oc_sku.var = apl.var)
				...
				WHERE apl.case_id IS NULL AND alr.marketplace = '" . $this->db->escape($marketplace) . "'
				ORDER BY alr.sku
			");
		} else {
			$result = $this->db->query("
	...
	public function install() {
		...
		$this->db->query("
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_order_case` (
				`order_case_id` int(11) NOT NULL ,
				`amazon_order_item_id` varchar(255) NOT NULL,
				PRIMARY KEY(`order_case_id`, `amazon_order_item_id`)
		);");

		$this->db->query("
			CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_case_unshipped` (
				`order_id` int(11) NOT NULL,
		...
		$this->db->query("
		CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_case` (
		...
		$this->db->query("
		CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_case_error` (
		...
		$this->db->query("
		CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_case_link` (
		...
		$this->db->query("
		CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "amazon_case_search` (
			`case_id` int(11) NOT NULL,
			`marketplace` enum('uk','de','es','it','fr') NOT NULL,
			`status` enum('searching','finished') NOT NULL,
			`matches` int(11) DEFAULT NULL,
			`data` text,
			PRIMARY KEY (`case_id`,`marketplace`)
		) DEFAULT COLLATE=utf8_general_ci;");
	...
	public function uninstall() {
		...
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_order_case`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case2`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case_link`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case_unshipped`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case_error`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_process`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case_unshipped`");
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "amazon_case_search`");
