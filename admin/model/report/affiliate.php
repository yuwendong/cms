...
class ModelReportAffiliate extends Model {
	...
	public function getCases($data = array()) { 
		$sql = "SELECT at.case_id, CONCAT(a.firstname, ' ', a.lastname) AS affiliate, a.email, a.status, SUM(at.amount) AS commission, COUNT(o.order_id) AS orders, SUM(o.total) AS total FROM " . DB_PREFIX . "affiliate_transaction at LEFT JOIN `" . DB_PREFIX . "affiliate` a ON (at.affiliate_id = a.affiliate_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (at.order_id = o.order_id) LEFT JOIN " . DB_PREFIX . "case";
	...
	public function getTotalCases() {
		$sql = "SELECT COUNT(DISTINCT case_id) AS total FROM `" . DB_PREFIX . "affiliate_transaction`";
