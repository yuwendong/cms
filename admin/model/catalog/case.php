...
class ModelCatalogCase extends Model {
	...
	public function editCase($case_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "case_related WHERE case_id = '" . (int)$case_id . "' AND type=0");
		$this->db->query("DELETE FROM " . DB_PREFIX . "case_related WHERE related_id = '" . (int)$case_id . "' AND type=0");
		if (isset($data['case_related'])) {
			foreach ($data['case_related'] as $related_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "case_related SET case_id = '" . (int)$case_id . "', related_id = '" . (int)$related_id . "',weight=1,type=0,invalid=0");
				$this->db->query("INSERT INTO " . DB_PREFIX . "case_related SET case_id = '" . (int)$related_id . "', related_id = '" . (int)$case_id . "',weight=1,type=0,invalid=0");
			}
		}
	...
	public function getCaseRelatedByTotal($case_id) {
		...
		$query = $this->db->query("SELECT case_id,related_id,sum(weight) as weight, COUNT( * ) AS count FROM " . DB_PREFIX . "case_related WHERE case_id = '" . (int)$case_id . "' and status='0' and invalid='0' group by related_id");
		return $query->rows;
	}
