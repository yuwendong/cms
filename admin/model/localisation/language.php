...
class ModelLocalisationLanguage extends Model {
	public function addLanguage($data) {
		...
		// Case
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "case_description WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $case) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "case_description SET case_id = '" . (int)$case['case_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($case['name']) . "', meta_description = '" . $this->db->escape($case['meta_description']) . "', meta_keyword = '" . $this->db->escape($case['meta_keyword']) . "', description = '" . $this->db->escape($case['description']) . "', tag = '" . $this->db->escape($case['tag']) . "'");
		}

		$this->cache->delete('case');

		// Case Attribute 
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "case_attribute WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'");

		foreach ($query->rows as $case_attribute) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "case_attribute SET case_id = '" . (int)$case_attribute['case_id'] . "', attribute_id = '" . (int)$case_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" . $this->db->escape($case_attribute['text']) . "'");
		}
	...
	public function deleteLanguage($language_id) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "case_attribute WHERE language_id = '" . (int)$language_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "case_description WHERE language_id = '" . (int)$language_id . "'");
